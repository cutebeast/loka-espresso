...
const ProductCard = memo(function ProductCard({
  ...
  return <div className="product-card">
      <div className="product-img">
        {imgSrc ? <img src={imgSrc} alt={t("home.promotions_section.promotion")} className="card-bg-img" loading="lazy" /> : <div className="home-img-fallback">
...
export default function PromotionsSection({
  ...
  return <div>
      <div className="home-section-header">
        ...
        <button onClick={onSeeAll} className="link home-see-all-link">
          {t('home.seeAll')} <ArrowRight size={12} />
        </button>
      </div>

      {loading ? <div className="product-scroll">
          {Array.from({
        length: 4
      }).map((_, i) => <div key={i} className="product-card home-skeleton-card">
              <div className="product-img">
        ...
        </div> : featuredItems.length === 0 ? <div className="home-empty">
          <div className="home-empty-icon"><Coffee size={32} strokeWidth={1.5} /></div>
          <p className="home-empty-text">{t('home.noItemsAvailable')}</p>
        </div> : <div className="product-scroll">
          {featuredItems.slice(0, 8).map(item => <ProductCard key={item.id} item={item} onAdd={() => onAddToCart(item)} />)}
        </div>}
    </div>;
}