...
export default function CheckinPage() {
  ...
  useEffect(() => {
    if (isAuthenticated) load();else setLoading(false);
  ...
  if (!isAuthenticated) {
    return <div className="home-empty" style={{
      paddingTop: 80
    }}>
  ...
  if (loading) {
    return <div className="skeleton home-skeleton-card" style={{
      height: 300,
      margin: 16
    }} />;
  }
      ...
      marginBottom: 16
        ...
        gap: 4
          ...
          return <div key={day} style={{
            ...
            height: 8,
            borderRadius: 4,
            background: filled ? `linear-gradient(90deg, ${LOKA.copper}, #C4893A)` : 'var(--loka-cream)',
      ...
      marginBottom: 16
        ...
        gap: 8
          ...
          fontSize: 13
            ...
            <span style={{
            color: 'var(--loka-text-muted)'
          }}>{t('checkin.dailyBase')}</span>
            <span style={{
            fontWeight: 700,
            color: LOKA.copper
          }}>+{cfg?.daily_base_points || 10}{t("checkin_page.pts")}</span>
          </div>
          <div style={{
          display: 'flex',
          justifyContent: 'space-between',
          fontSize: 13
            ...
            <span style={{
            color: 'var(--loka-text-muted)'
          }}>{t('checkin.streakBonus')}</span>
            <span style={{
            fontWeight: 700,
            color: LOKA.copper
          }}>+{cfg?.streak_increment || 2}{t("checkin_page.pts_day")}</span>
          </div>
          <div style={{
          display: 'flex',
          justifyContent: 'space-between',
          fontSize: 13
            ...
            <span style={{
            color: 'var(--loka-text-muted)'
          }}>{t('checkin.sevenDayBonus')}</span>
            <span style={{
            fontWeight: 700,
            color: LOKA.copper
          }}>+{cfg?.streak_7day_bonus || 20}{t("checkin_page.pts_2")}</span>
          </div>
        </div>
      </div>

      {/* Check-in Button */}
      {data?.checked_in_today ? result || data.checked_in_today ? <div style={{
      textAlign: 'center',
      padding: 20,
      borderRadius: 16,
      background: `linear-gradient(135deg, ${LOKA.copper}, #C4893A)`,
