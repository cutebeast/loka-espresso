...
export default function InformationPage({
  ...
  if (selectedCard) {
    ...
    return <div className="info-detail-screen">
        <div className="info-detail-hero">
          {allImages.length > 1 ? <ImageCarousel images={allImages} title={selectedCard.title} /> : <>
              {img && !brokenImages.has(selectedCard.id) ? <img src={img} alt={selectedCard.title} loading="lazy" className="info-detail-hero-img" onError={() => {
            setBrokenImages(prev => new Set(prev).add(selectedCard.id));
  ...
  return <div className="info-screen">
      ...
      <div className="info-tab-bar">
        {CONTENT_TYPES.map(t => <button key={t.id} className={`info-tab ${activeTab === t.id ? 'active' : ''}`} onClick={() => handleTabChange(t.id)}>
            {t.label}
          </button>)}
      </div>
      ...
      <div className="info-card-list">
        {loading ? <>
            {[1, 2, 3].map(i => <div key={i} className="skeleton info-skeleton-card" />)}
          </> : cards.length === 0 ? <div className="info-empty">
        ...
        return <div key={card.id} className="info-card" onClick={() => setSelectedCard(card)}>
                <div className="info-card-thumb">
                  {img && !brokenImages.has(card.id) ? <img src={img} alt={card.title || 'Information card'} loading="lazy" onError={() => {
              setBrokenImages(prev => new Set(prev).add(card.id));
                ...
                <div className="info-card-body">
                  ...
                  {card.short_description && <div className="info-card-desc">{card.short_description}</div>}
...
function ImageCarousel({
  ...
  return <>
      <div className="info-carousel-wrap" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} onMouseDown={onMouseDown} onMouseUp={onMouseUp}>
        <div ref={trackRef} className="info-carousel-track">
          {images.map((src, i) => <div key={i} className="info-carousel-slide" onClick={() => setLightbox(true)}>
              <img src={src} alt={title ? `${title} — image ${i + 1}` : `Image ${i + 1}`} className="info-carousel-img" loading={i === 0 ? 'eager' : 'lazy'} draggable={false} />
            </div>)}
        </div>

        {/* Dots */}
        {total > 1 && <div className="info-carousel-dots">
            {images.map((_, i) => <button key={i} className={`info-carousel-dot ${i === current ? 'active' : ''}`} onClick={e => {
          e.stopPropagation();
          setCurrent(i);
        ...
        {/* Arrow buttons */}
        {total > 1 && <>
            <button className="info-carousel-arrow info-carousel-arrow-left" onClick={prev} aria-label={t("information_page.previous")}>
              <ChevronLeft color="#8A8078" size={14} strokeWidth={3} />
            </button>
            <button className="info-carousel-arrow info-carousel-arrow-right" onClick={next} aria-label={t("information_page.next")}>
      ...
      {/* Lightbox */}
      {lightbox && <div className="info-lightbox-overlay" onClick={() => setLightbox(false)}>
          <img src={images[current]} alt={title ? `${title} — full view` : 'Full view'} className="info-lightbox-img" />
          <button className="info-lightbox-close" onClick={() => setLightbox(false)} aria-label={t("information_page.close_lightbox")}>
            <X size={20} />
          </button>
        </div>}
    </>;
