...
export function HubHeader({
    ...
    switch (variant) {
      ...
      case 'rewards':
        return <div className="flex flex-col gap-0.5">
            ...
            {points != null && <span className="text-xs font-medium text-text-muted">{t('profile.pts', {
  ...
  const renderRight = (): ReactNode => {
    ...
    if (variant === 'home' && onQRScanClick) {
      actions.push(<motion.button key="qr" whileTap={{
        scale: 0.9
      }} onClick={onQRScanClick} className="w-11 h-11 rounded-xl bg-copper-10 flex items-center justify-center cursor-pointer border border-copper-30" aria-label={t('home.scanQR')} title={t("ui.hub_header.scan_table_qr")}>
    ...
    if (variant === 'menu' && onSearchClick) {
      actions.push(<motion.button key="search" whileTap={{
        scale: 0.9
      }} onClick={onSearchClick} className="w-11 h-11 rounded-xl bg-surface flex items-center justify-center cursor-pointer border border-border-subtle" aria-label={t('menu.search')}>
    ...
    if (variant === 'orders' && onRefreshClick) {
      actions.push(<motion.button key="refresh" whileTap={{
        scale: 0.9
      }} onClick={onRefreshClick} className="w-11 h-11 rounded-xl bg-surface flex items-center justify-center cursor-pointer border border-border-subtle" aria-label={t('orders.tapToRefresh')}>
    ...
    if (variant === 'profile' && onSettingsClick) {
      actions.push(<motion.button key="settings" whileTap={{
        scale: 0.9
      }} onClick={onSettingsClick} className="w-11 h-11 rounded-xl bg-surface flex items-center justify-center cursor-pointer border border-border-subtle" aria-label={t('settings.title')}>
    ...
    if (variant !== 'profile') {
      actions.push(<NotificationBell key="notifications" unreadCount={unreadNotifications} onClick={onNotificationClick} />);
    }
    return <div className="flex items-center gap-2 shrink-0">
        {actions}
      </div>;
  };
  return <div className="bg-white border-b border-border-subtle">
