...
function SvgLogo({
  ...
  return <div className={`splash-logo-v2 ${reducedMotion ? '' : 'animate-fade-scale'}`}>
...
export function SplashScreen({
  ...
  useEffect(() => {
    ...
    api.get('/splash', {
      ...
      if (err?.name !== 'AbortError') console.error('[Splash] Content fetch failed:', err);
  ...
  if (loading) {
    return <div className="splash-page-v2">
  ...
  if (splashData) {
    return <div className="splash-page-v2">
        <div className="splash-page-inner-v2" style={{
        gap: 8
      }}>
          <div className={`splash-logo-v2 ${reducedMotion ? '' : 'animate-fade-scale'}`} style={{
          width: '100%',
          maxWidth: 280,
          height: 280
        }}>
            <img src={splashData.image_url} alt={splashData.title || 'Welcome'} style={{
            width: '100%',
            height: '100%',
            objectFit: 'contain',
            borderRadius: 16
          ...
          {splashData.title && <div className={`splash-title-v2 ${reducedMotion ? '' : 'animate-fade-up-1'}`}>
              {splashData.title}
            </div>}
          {splashData.subtitle && <div className={`splash-tagline-v2 ${reducedMotion ? '' : 'animate-fade-up-2'}`}>
              {splashData.subtitle}
            </div>}
          ...
          {splashData.dismissible && <button type="button" onClick={onFinish} className="splash-dismiss-btn" aria-label={t("auth.splash_screen.skip")}>{t("auth.splash_screen.skip_2")}</button>}
        </div>
      </div>;
  }
  ...
  return <div className="splash-page-v2">
      <div className="splash-page-inner-v2">
        ...
        <button type="button" onClick={onFinish} className="splash-dismiss-btn" aria-label={t("auth.splash_screen.skip_3")}>{t("auth.splash_screen.skip_4")}</button>
      </div>
    </div>;
}