...
import { ArrowLeft, Wallet, Banknote, CheckCircle2, Loader2, UtensilsCrossed, Coffee, Tag, QrCode, ChevronRight, Utensils, Store } from 'lucide-react';
...
const ORDER_TYPES = [{
  key: 'pickup' as const,
  labelKey: 'cart.mode.pickup'
}, {
  key: 'delivery' as const,
  labelKey: 'cart.mode.delivery'
}, {
  key: 'dine_in' as const,
  labelKey: 'cart.mode.dineIn'
}];
export default function CheckoutPage() {
  ...
  const [deliveryAddress, setDeliveryAddress] = useState<{
    ...
    lat?: number;
    lng?: number;
  } | null>(checkoutDraft.deliveryAddress ?? null);
  ...
  const [paymentMethod, setPaymentMethod] = useState<'wallet' | 'pay_at_store' | 'cod' | 'cash' | 'gateway' | 'hitpay'>(checkoutDraft.paymentMethod || (orderMode === 'dine_in' ? 'pay_at_store' : 'wallet'));
  ...
  }, [user, checkoutDraft.recipientName, recipientName]);
  ...
  useEffect(() => {
    ...
    if (!discountType && !discountCode) {
      if (checkoutDraft.voucherCode) {
        ...
        setDiscountCode(checkoutDraft.voucherCode);
      } else if (checkoutDraft.rewardCode) {
        migratedDraft.current = true;
        setDiscountType('reward');
        setDiscountCode(checkoutDraft.rewardCode);
      }
    } else {
      migratedDraft.current = true;
    }
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (missing.size > 0) {
        ...
        showToast(t('checkout.fieldErrors'), 'error');
        return;
      }
      if (deliveryOutOfRange) {
        showToast(t('toast.outOfRange'), 'error');
        return;
      }
      ...
      try {
        ...
        })?.response?.data?.detail || t('toast.orderFailed'), 'error');
      } finally {
        setPlacing(false);
      }
    } finally {
  ...
  return <>
    <div className="checkout-screen">
      ...
      <div className="checkout-scroll">
        <div className="checkout-section">
          ...
          <div className="co-type-pills">
            {ORDER_TYPES.map(ot => {
              ...
              return <button key={ot.key} className={`co-type-pill ${isCurrent ? 'active' : ''} ${isDineIn && !dineInSession ? 'disabled' : ''}`} onClick={() => {
                if (isDineInLocked && ot.key !== 'dine_in') {
                  showToast(t('toast.dineInLocked'), 'info');
                  return;
                }
                if (isClicable) setOrderMode(ot.key);else showToast(t('toast.dineInUnavailable'), 'info');
        ...
        {orderMode === 'dine_in' && dineInSession && <div className="checkout-section">
            <div className="co-section-title">{t('checkout.table')}</div>
            <div className="co-store-info"><div className="co-store-icon"><Utensils size={16} /></div><div><div className="co-store-name">{t('checkout.tableNumber', {
                  number: dineInSession.tableNumber
                })}</div><div className="co-store-address">{dineInSession.storeName}</div></div></div>
          </div>}
        {(orderMode === 'pickup' || orderMode === 'delivery') && selectedStore && <div className="checkout-section">
        ...
        {orderMode === 'delivery' && <div className={`checkout-section${fieldErrors.has('address') ? ' error' : ''}`}>
            <div className="co-section-title">{t('checkout.deliveryAddress')}</div>
            <div className="co-delivery-fields">
              <div className="co-delivery-field"><label className="co-delivery-label">{t('checkout.recipientName')}</label><input value={recipientName} onChange={e => {
                setRecipientName(e.target.value);
                saveDraft();
              }} placeholder={t('checkout.namePlaceholder')} autoComplete="name" className="co-delivery-input" /></div>
              <div className="co-delivery-field"><label className="co-delivery-label">{t('checkout.recipientPhone')}</label><input value={recipientPhone} onChange={e => {
                setRecipientPhone(e.target.value);
                saveDraft();
              }} placeholder="60123456789" autoComplete="tel" inputMode="tel" className="co-delivery-input" /></div>
            </div>
            <DeliveryAddressCard value={deliveryAddress} onChange={addr => {
            setDeliveryAddress(addr);
            saveDraft();
          }} />
            <div className="co-delivery-field mt-2"><label className="co-delivery-label">{t('checkout.deliveryInstructions')}</label><input value={deliveryInstr} onChange={e => {
              setDeliveryInstr(e.target.value);
              saveDraft();
            }} placeholder={t('checkout.deliveryInstrPlaceholder')} autoComplete="off" className="co-delivery-input" /></div>
            {deliveryOutOfRange && selectedStore && <div className="co-delivery-out-of-range">
                {t('checkout.outOfRange', {
              radius: selectedStore.delivery_radius_km ?? 0
            })}
              </div>}
          </div>}
        {orderMode !== 'dine_in' && <div className={`checkout-section${fieldErrors.has('time') ? ' error' : ''}`}>
            <div className="co-section-title">{orderMode === 'pickup' ? t('checkout.pickupTime') : t('checkout.deliveryTime')}</div>
            <TimeSlotPicker onChange={t => {
            setPickupTime(t);
            saveDraft();
          }} value={pickupTime} mode={orderMode === 'delivery' ? 'delivery' : 'pickup'} leadMinutes={selectedStore?.pickup_lead_minutes ?? config.pickup_lead_minutes} />
          </div>}
        <div className="checkout-section">
        ...
        <div className="checkout-section">
          ...
          {orderMode !== 'dine_in' && <div className="co-wallet-balance" onClick={() => setPaymentMethod('wallet')}>
          ...
          {stripeReady && <div className={`co-payment-card ${paymentMethod === 'gateway' ? 'selected' : ''}`} onClick={() => setPaymentMethod('gateway')}>
              <div className="co-payment-icon co-payment-icon-gateway"><Banknote size={14} color="#fff" /></div>
              <div className="co-payment-info"><div className="co-payment-label">{t("checkout_page.pay_online_card_fpx_grabpay")}</div></div>
              <div className="co-payment-check"><CheckCircle2 size={12} /></div>
            </div>}
          {hitpayEnabled && <div className={`co-payment-card ${paymentMethod === 'hitpay' ? 'selected' : ''}`} onClick={() => setPaymentMethod('hitpay')}>
              <div className="co-payment-icon co-payment-icon-gateway"><QrCode size={14} color="#fff" /></div>
              <div className="co-payment-info"><div className="co-payment-label">{t("checkout_page.pay_online_duitnow_tng_boost_shopeepay")}</div></div>
              <div className="co-payment-check"><CheckCircle2 size={12} /></div>
            </div>}
        </div>
        <div className="co-summary-card">
          ...
          <div className="co-order-items-list">
            {items.map((item, _i) => {
              ...
              const tags = cust?.options?.map(o => {
                const name = o.name || '';
                const colonIdx = name.indexOf(': ');
                return colonIdx >= 0 ? name.slice(colonIdx + 2) : name;
              }) || [];
              return <div key={`${item.menu_item_id}-${JSON.stringify([...(item.customization_option_ids ?? [])].sort())}`} className="co-order-item-row">
      ...
      <div className="checkout-footer">
        <div className="checkout-footer-row">
          ...
          <div className="checkout-footer-count">{t('cart.itemCount', {
              count: itemCount
            })}</div>
        </div>
        {requiresWallet && !walletSufficient ? <button className="co-topup-btn" onClick={() => setPage('wallet')}><Wallet size={18} /> {t('checkout.topUpToContinue', {
            amount: formatPrice(total - balance)
          })}</button> : <button className="co-place-order-btn" onClick={handlePlaceOrder} disabled={placing}>{placing ? <Loader2 size={18} className="spinning" /> : <UtensilsCrossed size={18} />}{placing ? t('checkout.placingOrder') : t('checkout.placeOrderPrice', {
    ...
    <BottomSheet isOpen={showRewardSheet} onClose={() => setShowRewardSheet(false)} title={t('checkout.voucherRewards')}>
      <div className="sheet-body">
        <VoucherRewardSelector subtotal={subtotal} selectedType={discountType || 'none'} selectedCode={discountCode} onChange={(type, code, val) => {
          setDiscountType(type === 'none' ? null : type);
          setDiscountCode(code || '');
          setDiscountValue(val || 0);
        }} />
      </div>
    </BottomSheet>
    <BottomSheet isOpen={!!gatewayIntent} onClose={() => {
      setGatewayIntent(null);
      setGatewayOrder(null);
    }} title={t('checkout.payOnline')}>
      <div className="sheet-body">
        {gatewayIntent && gatewayOrder && stripePublishableKey && <StripePaymentSheet clientSecret={gatewayIntent.clientSecret} paymentId={gatewayIntent.paymentId} orderId={gatewayOrder.id} publishableKey={stripePublishableKey} onSuccess={() => {
          clearCheckoutDraft();
          haptic('success');
          setGatewayIntent(null);
          setGatewayOrder(null);
