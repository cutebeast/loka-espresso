...
const pageImporters: Record<string, () => Promise<{
...
(function initDynamicCache() {
  for (const [key, importer] of Object.entries(pageImporters)) {
    dynamicCache[key] = dynamic(importer, {
      ssr: false
    });
  }
})();
...
export default function AppShell() {
  ...
  useEffect(() => {
    if ((showStoreModal || showStorePicker) && stores.length === 0) {
      ...
      api.get('/stores', {
        ...
        if ((err as {
          name?: string;
        })?.name === 'CanceledError') return;
        console.error('[AppShell] Store list fetch failed:', err);
        showToast(t('toast.storesLoadFailed'), 'error');
      });
  ...
  const renderPage = useMemo(() => {
    switch (page) {
      case 'home':
        return <HubLayout page={page} onNavigate={handleNavClick} header={<HomeHeader userName={user?.display_name} unreadNotifications={unreadCount} onNotificationClick={onNotificationClick} onQRScanClick={onQRScanClick} />}>
          <HomePage />
        </HubLayout>;
      case 'menu':
        return <HubLayout page={page} onNavigate={handleNavClick}>
          <MenuPage />
        </HubLayout>;
      case 'rewards':
        return <HubLayout page={page} onNavigate={handleNavClick}>
      ...
      case 'cart':
        return <CartPage />;
      case 'checkout':
        return <LazyRenderer pageKey="checkout" />;
      case 'wallet':
        return <LazyRenderer pageKey="wallet" />;
      case 'history':
        return <LazyRenderer pageKey="history" />;
      case 'promotions':
        return <HubLayout page={page} onNavigate={handleNavClick}>
          <LazyRenderer pageKey="promotions" onBack={onBackHome} preselectedId={pageParams.selectedPromoId as number | undefined} />
        </HubLayout>;
      case 'information':
        return <LazyRenderer pageKey="information" onBack={onBackHome} preselectedId={pageParams.selectedInfoId as number | undefined} preselectedSlug={pageParams.selectedInfoSlug as string | undefined} contentType={pageParams.selectedInfoContentType as string | undefined} />;
      case 'my-rewards':
        return <LazyRenderer pageKey="my-rewards" onBack={onBackProfile} initialTab={pageParams.initialTab as 'rewards' | 'vouchers' | undefined} />;
      case 'account-details':
        return <LazyRenderer pageKey="account-details" />;
      case 'payment-methods':
        return <LazyRenderer pageKey="payment-methods" />;
      case 'saved-addresses':
        return <LazyRenderer pageKey="saved-addresses" />;
      case 'notifications':
        return <LazyRenderer pageKey="notifications" />;
      case 'help-support':
        return <LazyRenderer pageKey="help-support" />;
      case 'legal':
        return <LazyRenderer pageKey="legal" />;
      case 'settings':
        return <LazyRenderer pageKey="settings" />;
      case 'referral':
        return <LazyRenderer pageKey="referral" />;
      case 'my-card':
        return <LazyRenderer pageKey="my-card" />;
      case 'order-detail':
        return <LazyRenderer pageKey="order-detail" />;
      case 'reservations':
        return <LazyRenderer pageKey="reservations" onBack={onBackProfile} />;
      case 'events':
        return <LazyRenderer pageKey="events" onBack={onBackProfile} />;
      case 'checkin':
        return <LazyRenderer pageKey="checkin" onBack={onBackHome} />;
      default:
        return <HomePage />;
    }
  ...
  return <div className="app-container">
      <OfflineBanner />
      {swUpdateAvailable && <motion.div initial={{
      opacity: 0,
      y: -40
    }} animate={{
      opacity: 1,
      y: 0
          ...
          <button className="sw-update-btn" onClick={async () => {
      ...
      {!isGuest && !authDone ? <AuthFlow onAuthDone={handleAuthDone} /> : <>
          {/* Toast */}
          ...
          duration: 0.2
        }} className="a2hs-backdrop" onClick={a2hs.dismiss} />
              {/* Modal */}
              <motion.div initial={{
          opacity: 0,
          scale: 0.92
          ...
          duration: 0.25
                ...
                <div className="a2hs-modal-box">
                  ...
                  <div className="a2hs-app-row">
                    <img src="/icon-192.png" alt={t("app_shell.loka")} className="a2hs-app-icon" />
                    <div>
                      <p className="a2hs-app-name">{t("app_shell.loka_espresso")}</p>
                      <p className="a2hs-app-url">{appHost}</p>
                    </div>
                  </div>
                  <div className="a2hs-btn-row">
                    <button onClick={a2hs.dismiss} className="a2hs-btn-cancel">
                      {t('common.cancel')}
                    </button>
                    <button onClick={a2hs.promptInstall} className="a2hs-btn-add">
                      {t('common.add')}
                    </button>
                  </div>
                </div>
          ...
          {/* Store Picker Modal */}
          <AnimatePresence>
            {(showStoreModal || showStorePicker) && <StorePickerModal stores={stores} selectedStore={selectedStore} userLocation={userLocation} onSelect={store => {
          ...
          {/* QR Scanner */}
          <LazyRenderer pageKey="qrScanner" isOpen={showQRScanner} onClose={() => setShowQRScanner(false)} onScan={async (result: string) => {
        ...
        try {
          const res = await api.post('/stores/tables/scan', {
          ...
          showToast(t('toast.tableScanned', {
          ...
          })?.response?.data?.detail || t('toast.scanTableFailed');
          showToast(msg, 'error');
        }
      }} />

          {/* Login Modal — shown when guest triggers sign-in */}
          <LoginModal isOpen={showLoginModal} onClose={() => setShowLoginModal(false)} onAuthDone={handleAuthDone} />
        </>}
