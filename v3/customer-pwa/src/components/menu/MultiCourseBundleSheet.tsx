...
export default function MultiCourseBundleSheet({
  bundle,
  onClose,
  onDone
}: MultiCourseBundleSheetProps) {
  const {
    t
  } = useTranslation();
  ...
  const handleAddToCart = () => {
    ...
    const msg = totalExtraPrice > 0 ? `${bundle.title} added (+${formatPrice(totalExtraPrice)} extras)` : `${bundle.title} added`;
  ...
  return <AnimatePresence>
      ...
      if (e.target === e.currentTarget) onClose();
    }} role="dialog" aria-modal="true" aria-labelledby="mcb-title">
        ...
        stiffness: 300
          ...
          marginBottom: 8
            ...
            return <button key={gi} onClick={() => setCurrentGroupIdx(gi)} style={{
              padding: "8px 14px",
              fontSize: 12,
              fontWeight: currentGroupIdx === gi ? 700 : 400,
              border: "none",
              borderBottom: currentGroupIdx === gi ? "3px solid var(--color-primary)" : "3px solid transparent",
          ...
          {currentGroup && <>
            ...
            alignItems: "center"
                ...
                margin: "2px 0 0"
                  ...
                  selected: currentSelections.size,
                  count: pickCount
                })}
                    {maxPick > pickCount && ` (up to ${maxPick})`}
                  </p>
                </div>
                {currentSelections.size >= pickCount && currentSelections.size < maxPick && <span style={{
              fontSize: 11,
              color: "var(--color-warning)"
            }}>+{getCurrencySymbol()}{t("menu.multi_course_bundle_sheet.each_extra")}</span>}
              </div>

              <div className="bps-scroll" style={{
            flex: 1
          }}>
                ...
                return <div key={item.componentId} className={`bps-item-card ${selected ? 'bps-selected' : ''} ${!selectable && !selected ? 'bps-dimmed' : ''}`} onClick={() => toggleItem(item.componentId)} onKeyDown={e => {
                  if (e.key === 'Enter' || e.key === ' ') {
                    ...
                    toggleItem(item.componentId);
                        ...
                        <div className="bps-item-img">
                          ...
                          {selected && <div className="bps-check-badge">
                              <Check size={12} />
                            </div>}
                        </div>
          ...
          <div className="bps-footer" style={{
          display: "flex",
          gap: 8,
          alignItems: "center"
        }}>
            <div className="bps-footer-price" style={{
            flex: 1
          }}>
            ...
            {isLastGroup ? <button className={`bps-add-btn ${canComplete ? '' : 'bps-add-btn-disabled'}`} disabled={!canComplete} onClick={handleAddToCart}>
                {canComplete ? t('menu.pickXAddToCart') : `${currentSelections.size}/${pickCount} selected`}
              </button> : <button className={`bps-add-btn ${isGroupSatisfied ? '' : 'bps-add-btn-disabled'}`} disabled={!isGroupSatisfied} onClick={nextGroup} style={{
            display: "flex",
            alignItems: "center",
            gap: 4
          }}>{t("menu.multi_course_bundle_sheet.next")}<ArrowRight size={16} />
              </button>}
          </div>
        </motion.div>
      </motion.div>
