...
function paymentIndicator(order: Order): {
  color: string;
  label: string;
} | null {
  if (isPaid(order.payment_status)) return {
    color: "#16A34A",
    label: "Paid"
  };
...
export default function OrderCard({
  ...
  return <div role="button" tabIndex={0} className={`bg-white rounded-xl border p-4 shadow-sm hover:shadow-md transition cursor-pointer ${urgency === "overdue" ? "ring-red-300 animate-pulse" : ""} ${compact ? "" : "mb-3"}`} style={urgency !== "normal" ? {
    borderColor: urgencyStyle.border,
    backgroundColor: urgencyStyle.background
  } : undefined} onClick={() => onClick?.(order)} onKeyDown={e => {
    if (e.key === "Enter" || e.key === " ") {
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {order.table_number && <span className="badge badge-sm badge-primary">{t("common.table")}{order.table_number}</span>}
          {payInd && <span className="badge badge-sm flex items-center gap-1" style={{
          background: payInd.color + "15",
          color: payInd.color,
          borderColor: payInd.color + "40"
        }}>
              <span style={{
            width: 6,
            height: 6,
            borderRadius: "50%",
            background: payInd.color,
      ...
      <div className="space-y-2 mb-3">
        {visibleItems.map((item, idx) => <div key={`${item.menu_item_id || item.id || item.name}-${idx}`} className="flex items-center justify-between text-sm">
            <span className="text-gray-700">
              <span className="font-medium">{item.quantity}{t("common.x")}</span> {item.name}
              {item.modifiers_label && <span className="text-gray-400 text-xs ml-1">({item.modifiers_label})</span>}
            </span>
          </div>)}
        {hiddenCount > 0 && <p className="text-xs text-gray-400">+{hiddenCount}{t("common.more_items")}</p>}
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-100 text-xs">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {order.customer_name && <span className="flex items-center gap-1">
              <User size={12} />
              {order.customer_name}
            </span>}
        </div>
        <div className="flex items-center gap-3">
          <span className="font-semibold text-gray-700">{t("common.rm")}{typeof order.total_amount === "number" && !isNaN(order.total_amount) ? order.total_amount.toFixed(2) : "0.00"}</span>
          {next && onQuickAction && <button type="button" onClick={e => {
          e.stopPropagation();
          onQuickAction(order, next);
