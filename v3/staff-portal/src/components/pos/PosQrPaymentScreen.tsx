...
export default function PosQrPaymentScreen({
  ...
  useEffect(() => {
    let mounted = true;
    import("qrcode").then(QRCode => QRCode.toDataURL(paymentUrl, {
      width: 280,
      margin: 2,
      color: {
        dark: "#2C1E16",
  ...
  return <div style={{
    ...
    textAlign: "center"
      ...
      fontWeight: 700,
      marginBottom: 8
    }}>{t("pos.scan_to_pay")}</h2>
      {orderNumber && <p style={{
      color: "var(--color-text-muted)",
      marginBottom: 16
    }}>{t("pos.order")}{orderNumber}</p>}
      <div style={{
      fontSize: 28,
      fontWeight: 700,
      marginBottom: 24
    }}>{t("pos.rm")}{total.toFixed(2)}</div>
      ...
      justifyContent: "center"
        ...
        {qrDataUrl ? <img src={qrDataUrl} alt={t("pos.payment_qr_code")} style={{
        width: "100%",
        height: "100%"
      }} /> : <div style={{
        color: "var(--color-text-muted)"
      ...
      marginBottom: 24
        ...
        {status === "paid" ? <>
          ...
          fontWeight: 700
        }}>{t("pos.payment_confirmed")}</span>
          </> : status === "failed" ? <>
            <XCircle size={20} color="var(--color-error)" />
            <span style={{
          color: "var(--color-error)",
          fontWeight: 700
        }}>{error || "Payment failed"}</span>
          </> : <>
            <Smartphone size={20} color="var(--color-info)" />
            <span style={{
          color: "var(--color-text-muted)"
        }}>{t("pos.waiting_for_customer_to_scan_and")}</span>
          </>}
      </div>

      {status !== "paid" && <button className="btn btn-ghost" onClick={onCancel}>{t("pos.cancel_qr_payment")}</button>}
    </div>;
}