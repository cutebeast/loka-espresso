...
export default function BundlePickerModal({
  ...
  const totalSelected = allowDuplicates ? Object.values(quantities).reduce((sum, q) => sum + q, 0) : currentSelections.size;
  const canAdd = isMultiCourse ? groups.every((g, gi) => {
  ...
  const handleAdd = () => {
    ...
    if (isMultiCourse) {
      ...
      for (const [componentIdStr, qty] of Object.entries(quantities)) {
        ...
        selections.push({
          menu_item_id: comp?.menu_item_id ?? 0,
          quantity: qty,
  ...
  const modalTitle = isMultiCourse ? `Build ${bundle?.title || 'Combo'}` : `Pick ${pickCount} for ${bundle?.title || 'Combo'}`;
  return <Modal open={open} onClose={onClose} title={modalTitle} size="lg" footer={<div style={{
    display: "flex",
    ...
    width: "100%"
      ...
      fontWeight: 600
          ...
          {isMultiCourse ? `${groups.filter((g, gi) => (groupSelections[gi] || new Set<number>()).size >= g.pick_count).length}/${groups.length} groups done` : `${totalSelected} / ${pickCount} selected`}
        </span>
        <div style={{
      display: "flex",
      gap: 8
    }}>
          <button type="button" className="btn btn-ghost" onClick={onClose}>{t("pos.cancel")}</button>
          <button type="button" className="btn btn-primary" disabled={!canAdd} onClick={handleAdd}>{t("pos.add_to_cart_rm")}{bundle?.bundle_price?.toFixed(2) ?? '0.00'}
          </button>
        </div>
      </div>}>
      <div style={{
      ...
      padding: "0 4px"
        ...
        {isMultiCourse && groups.length > 0 && <div style={{
        display: "flex",
        gap: 4,
        marginBottom: 12,
        flexWrap: "wrap"
          ...
          return <button key={gi} type="button" onClick={() => setCurrentGroupIdx(gi)} style={{
            padding: "6px 12px",
            fontSize: 11,
            fontWeight: currentGroupIdx === gi ? 600 : 400,
            border: currentGroupIdx === gi ? "1px solid var(--color-primary)" : "1px solid var(--color-border-light)",
            borderRadius: 20,
            background: currentGroupIdx === gi ? "rgba(59,74,26,0.08)" : "var(--color-bg-white)",
            cursor: "pointer",
            whiteSpace: "nowrap",
            color: done ? "var(--color-success)" : "inherit"
          }}>
                  {done && "✓ "}{g.group_label} ({sel.size}/{g.pick_count})
                </button>;
        })}
          </div>}
        {isMultiCourse && groups[currentGroupIdx] && <div style={{
        marginBottom: 12
      }}>
            <div style={{
          display: "flex",
          ...
          alignItems: "center"
              ...
              fontWeight: 700,
              margin: 0
            }}>{groups[currentGroupIdx].group_label}</h4>
                <p style={{
              fontSize: 11,
              color: "var(--color-text-muted)",
              margin: "2px 0 0"
            }}>{t("pos.pick")}{groups[currentGroupIdx].pick_count}{t("pos.up_to")}{groups[currentGroupIdx].max_pick})
                </p>
              </div>
              <div style={{
            display: "flex",
        ...
        {!isMultiCourse && !allowDuplicates && <p style={{
        fontSize: 12,
        color: "var(--color-text-muted)",
        marginBottom: 12
      }}>
            {selectedIds.size >= pickCount ? "Maximum selected. Remove one to pick another." : `Select ${pickCount} items to continue`}
          </p>}
        <div style={{
        display: "grid",
        ...
        gap: 10
          ...
          return <div key={item.componentId} onClick={() => toggleItem(item.componentId)} onKeyDown={e => {
            if (e.key === 'Enter' || e.key === ' ') {
              e.preventDefault();
              toggleItem(item.componentId);
            }
            ...
            position: "relative"
              ...
              background: "var(--color-bg-muted)"
                ...
                {selected && !allowDuplicates && <div style={{
              position: "absolute",
              top: 8,
              right: 8,
              width: 22,
            ...
            }}>{t("pos.rm")}{Number(item.price ?? 0).toFixed(2)}
                </div>

                {allowDuplicates && <div style={{
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              gap: 8
                ...
                opacity: qty > 0 ? 1 : 0.3
              }} disabled={qty <= 0} onClick={() => decQty(item.componentId)}>
                      <Minus size={14} />
                    </button>
                    <span style={{
                fontWeight: 700,
              ...
              }} disabled={totalSelected >= pickCount} onClick={() => incQty(item.componentId)}>
                      <Plus size={14} />
                    </button>
                  </div>}
              </div>;
