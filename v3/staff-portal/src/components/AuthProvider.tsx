...
export default function AuthProvider({
  ...
  useEffect(() => {
    ...
    return () => {
      if (injectedLink) injectedLink.remove();
    };
  }, []);
  ...
  return <div className="app-shell">
      ...
      gap: 12
    }}>{t("common.session_expiring_soon_tap_anywhere_to")}<button type="button" onClick={() => {
        setShowIdleWarning(false);
        window.dispatchEvent(new Event("staff:activity"));
      }} style={{
        background: "#856404",
        color: "white",
        border: "none",
      ...
      <main className="app-main">
        ...
        {forbidden ? <div className="forbidden-screen">
            <div>
              <p className="forbidden-title">{t("common.access_denied")}</p>
              <p className="forbidden-desc">{t("common.this_account_does_not_have_staff")}<br />{t("common.redirecting_to_login")}</p>
            </div>
          </div> : authError ? <div className="forbidden-screen">
            <div>
              <p className="forbidden-title">{t("common.connection_error")}</p>
              <p className="forbidden-desc">{t("common.unable_to_reach_the_server_please")}</p>
          ...
          </div> : checked ? children : <div className="loading-screen">
