...
const SKIP_FILES = [
  "src/app/login/page.tsx", // already translated
  "src/app/layout.tsx",
];
...
const PREFIX_MAP = {
  ...
  "src/app/kitchen/page.tsx": "kitchen",
  "src/app/kitchen/[id]/page.tsx": "kitchen",
  "src/app/tables/page.tsx": "tables",
  "src/app/reservations/page.tsx": "reservations",
  "src/app/wallet/page.tsx": "wallet",
  "src/app/time-clock/page.tsx": "time_clock",
  "src/app/shifts/page.tsx": "shifts",
  "src/app/tasks/page.tsx": "tasks",
  "src/app/profile/page.tsx": "profile",
  "src/app/inventory/page.tsx": "inventory",
  "src/app/wastage/page.tsx": "wastage",
  "src/app/equipment/page.tsx": "equipment",
  "src/app/garbage/page.tsx": "garbage",
  "src/app/grease-trap/page.tsx": "grease_trap",
  "src/app/error.tsx": "error",
  "src/app/loading.tsx": "loading",
};

function getPrefix(relPath) {
  if (PREFIX_MAP[relPath]) return PREFIX_MAP[relPath];
  if (relPath.startsWith("src/components/pos/")) return "pos";
  if (relPath.startsWith("src/components/")) return "common";
  return "common";
}
...
function chooseIdent(fnNode, preferred) {
  // If the function scope already binds 't', use '_t' for translation; if '_t' also bound, use '__t'.
  const candidates = [preferred, "_t", "__t", "___t"];
  for (const name of candidates) {
    if (fnNode._bindings && fnNode._bindings.has(name)) continue;
    return name;
  }
  return preferred;
}
...
function processFile(filePath, allKeys) {
  ...
  // Map function node -> { ident, count }
  const fnTranslationUsage = new Map();

  // Collect existing bindings per function to avoid shadowing
  ...
  traverse(ast, {
    "FunctionDeclaration|FunctionExpression|ArrowFunctionExpression|ObjectMethod"(path) {
      ...
      // body variable declarations at immediate scope
  ...
  function getIdentForPath(nodePath) {
    ...
    if (!fnNode) return "t";
    // If this function already has translation usage, reuse ident
    if (fnTranslationUsage.has(fnNode)) return fnTranslationUsage.get(fnNode).ident;
    // Choose ident that doesn't shadow existing bindings
    return chooseIdent(fnNode, "t");
  }
