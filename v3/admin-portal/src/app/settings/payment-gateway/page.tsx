...
const STRIPE_KEYS = [
  { key: "stripe.enabled", label: "Enable Stripe", type: "boolean" },
  { key: "stripe.secret_key", label: "Secret Key", type: "password" },
  { key: "stripe.publishable_key", label: "Publishable Key", type: "password" },
  { key: "stripe.webhook_secret", label: "Webhook Secret", type: "password" },
  { key: "stripe.checkout_success_url", label: "Checkout Success URL", type: "text" },
  { key: "stripe.checkout_cancel_url", label: "Checkout Cancel URL", type: "text" },
];

const HITPAY_KEYS = [
  { key: "hitpay.enabled", label: "Enable HitPay", type: "boolean" },
  { key: "hitpay.api_key", label: "API Key", type: "password" },
  { key: "hitpay.salt", label: "Webhook Salt", type: "password" },
  { key: "hitpay.api_base_url", label: "API Base URL", type: "text" },
];

const PUBLIC_URL_KEYS = [
  { key: "app.public_url", label: "Customer App Public URL", type: "text" },
  { key: "admin.public_url", label: "Admin Portal Public URL", type: "text" },
...
const LABEL_MAP: Record<string, string> = Object.fromEntries(
  [...STRIPE_KEYS, ...HITPAY_KEYS, ...PUBLIC_URL_KEYS].map(cfg => [cfg.key, cfg.label])
);
...
function MethodChips({ options, selected, onChange }: { options: string[]; selected: string[]; onChange: (v: string[]) => void }) {
  return (
...
export default function PaymentGatewaySettingsPage() {
  ...
  const { validationErrors, invalidKeys } = useMemo(() => {
    const errors: string[] = [];
    ...
    return { validationErrors: errors, invalidKeys: invalid };
  }, [values, methodValues]);
    ...
    try {
      ...
      for (const [key, methods] of Object.entries(methodValues)) {
        ...
        if (methods.length !== original.length || methods.some((m, i) => m !== original[i])) {
          updates.push({ key, value: JSON.stringify(methods) });
        }
      }
  ...
  const renderField = (cfg: { key: string; label: string; type: string }) => {
    ...
    if (cfg.type === "boolean") {
      return (
        <Toggle
          checked={parseBoolean(values[cfg.key] || "false")}
          onChange={v => setValues(prev => ({ ...prev, [cfg.key]: v ? "true" : "false" }))}
          label={cfg.label}
        />
      );
    }
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <label style={{ fontSize: 13, fontWeight: 500 }}>{cfg.label}</label>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {isSecret && (
            <button
              type="button"
              onClick={() => setVisibleSecrets(prev => ({ ...prev, [cfg.key]: !prev[cfg.key] }))}
      ...
      <div className="page-header" style={{ marginBottom: 24 }}>
        <div>
          ...
          <p className="page-subtitle">
            Manage Stripe and HitPay credentials, enabled methods, and redirect URLs. Changes take effect immediately.
          </p>
        </div>
      </div>

      {msg && (
        <div className={`alert ${isError ? "alert-error" : "alert-success"}`} style={{ marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 24, maxWidth: 720 }}>
        ...
        <div className="card" style={{ padding: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
            <CreditCard size={22} />
            <h2 style={{ fontSize: 17, fontWeight: 600 }}>Stripe</h2>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {STRIPE_KEYS.map(cfg => (
              <div key={cfg.key}>{renderField(cfg)}</div>
            ))}
            <div>
              <label style={{ fontSize: 13, fontWeight: 500, display: "block", marginBottom: 8 }}>Payment Methods</label>
              <MethodChips
                options={STRIPE_METHOD_OPTIONS}
                selected={methodValues["stripe.payment_method_types"] || []}
                onChange={v => setMethodValues(prev => ({ ...prev, "stripe.payment_method_types": v }))}
              />
        ...
        <div className="card" style={{ padding: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
            <Globe size={22} />
            <h2 style={{ fontSize: 17, fontWeight: 600 }}>Public URLs</h2>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {PUBLIC_URL_KEYS.map(cfg => (
              <div key={cfg.key}>{renderField(cfg)}</div>
            ))}
          </div>
          <p style={{ fontSize: 12, color: "var(--color-text-muted)", marginTop: 16 }}>
            Update these before moving to a new domain (e.g. lokaespresso.com). They control redirect URLs for Stripe and HitPay checkouts.
          </p>
        </div>
        ...
        <div className="card" style={{ padding: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
            <Wallet size={22} />
            <h2 style={{ fontSize: 17, fontWeight: 600 }}>HitPay</h2>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {HITPAY_KEYS.map(cfg => (
              <div key={cfg.key}>{renderField(cfg)}</div>
            ...
            <div>
              <label style={{ fontSize: 13, fontWeight: 500, display: "block", marginBottom: 8 }}>Payment Methods</label>
              <MethodChips
                options={HITPAY_METHOD_OPTIONS}
                selected={methodValues["hitpay.payment_methods"] || []}
                onChange={v => setMethodValues(prev => ({ ...prev, "hitpay.payment_methods": v }))}
              />
              <p style={{ fontSize: 12, color: "var(--color-text-muted)", marginTop: 8 }}>
        ...
        {validationErrors.length > 0 && (
          <div className="alert alert-error" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <AlertCircle size={18} />
              <strong>Please complete all required fields before saving</strong>
            </div>
            <ul style={{ margin: 0, paddingLeft: 24, fontSize: 13 }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          {!hasChanges && <span style={{ fontSize: 13, color: "var(--color-text-muted)" }}>No changes</span>}
        </div>
      </div>
    </div>
  );
