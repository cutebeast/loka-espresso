...
export default function CategoryEditPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      for (const loc of LOCALES) {
        ...
        const r = await api.getRaw<{items:{translation_key:string;translated_text:string}[]}>(`/translations?table_name=inventory_categories&record_id=${catId}&locale=${loc.code}&per_page=10`);
  ...
  const upsertTr = async (field: string, locale: string, src: string, text: string) => {
    const r = await api.getRaw<{items:{id:number}[]}>(`/translations?table_name=inventory_categories&record_id=${catId}&column_name=${field}&locale=${locale}&per_page=1`);
    const ex = r?.items?.[0];
    if (ex) await api.put(`/translations/${ex.id}`, { translated_text: text });
    else await api.post("/translations", { translation_key: `inventory_categories.${catId}.${field}`, locale, namespace: "inventory", translated_text: text, source_text: src, table_name: "inventory_categories", record_id: Number(catId), column_name: field });
  };
