...
export default function CategoryEditPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setForm({ category_name: d.category_name || "", slug: d.slug || "", description: d.description || "", is_active: d.is_active });
      const tr: Record<string,string> = {};
      for (const loc of LOCALES) {
        if (loc.code === "en") continue;
        const r = await api.getRaw<{items:{translation_key:string;translated_text:string}[]}>(`/admin/translations?table_name=inventory_categories&record_id=${catId}&locale=${loc.code}&per_page=10`);
  ...
  const upsertTr = async (field: string, locale: string, src: string, text: string) => {
    const r = await api.getRaw<{items:{id:number}[]}>(`/admin/translations?table_name=inventory_categories&record_id=${catId}&column_name=${field}&locale=${locale}&per_page=1`);
    const ex = r?.items?.[0];
    if (ex) await api.put(`/admin/translations/${ex.id}`, { translated_text: text });
    else await api.post("/admin/translations", { translation_key: `inventory_categories.${catId}.${field}`, locale, namespace: "inventory", translated_text: text, source_text: src, table_name: "inventory_categories", record_id: Number(catId), column_name: field });
  };
    ...
    <div style={{padding:32}}>
      ...
      {activeLocale==="en" ? (
        <div className="card" style={{padding:24,maxWidth:500}}>
          ...
          <div className="df-actions" style={{marginTop:16}}><button type="button" onClick={()=>router.push("/inventory/categories")} className="btn btn-ghost">Cancel</button><button onClick={save} disabled={saving} className="btn btn-primary"><Save size={16}/>{saving?"Saving...":"Save"}</button></div>
        </div>
      ) : (
