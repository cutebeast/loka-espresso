...
export default function NewSupplierPage() {
  ...
  const [form, setForm] = useState({ supplier_name: "", contact_person: "", phone_number: "", is_active: true });
  ...
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setSaving(true);
    try { await api.post("/admin/inventory/suppliers", { ...form, store_id: Number(storeId) }); router.push("/inventory/suppliers"); }
  ...
  return (
    <div style={{ padding: 32 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <div><h1 className="page-title" style={{ margin: 0 }}>New Supplier</h1></div>
      </div>
      ...
      <div className="card" style={{ padding: 24, maxWidth: 500 }}>
        ...
        </div><div className="df-actions" style={{ marginTop: 20 }}><button type="button" onClick={() => router.push("/inventory/suppliers")} className="btn btn-ghost">Cancel</button><button type="submit" disabled={saving} className="btn btn-primary"><Save size={16} /> {saving ? "Creating..." : "Create Supplier"}</button></div></form>
      </div>
    </div>
  );
}
