...
export default function InventoryItemEditPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      for(const lc of LOCALES){if(lc.code==="en")continue;try{const rt=await api.getRaw<any>(`/translations?table_name=inventory_items&record_id=${itemId}&locale=${lc.code}&per_page=50`);if(rt?.items)for(const t of rt.items){const f=t.translation_key.split(".").pop()||"";x[`${lc.code}:${f}`]=t.translated_text||"";}}catch{}}
      setTranslations(x);
  ...
  const upsertTr = async(field:string,locale:string,src:string,text:string)=>{
    try{const rt=await api.getRaw<any>(`/translations?table_name=inventory_items&record_id=${itemId}&column_name=${field}&locale=${locale}&per_page=1`);const ex=rt?.items?.[0];if(ex)await api.put(`/translations/${ex.id}`,{translated_text:text});else await api.post("/translations",{translation_key:`inventory_items.${itemId}.${field}`,locale,namespace:"inventory",translated_text:text,source_text:src,table_name:"inventory_items",record_id:Number(itemId),column_name:field});}catch{}
  };

  const regenerateAll = async () => {
    setRegenerating(true);let count=0;
    for(const f of TR_FIELDS){const src=(form[f.key]||"").trim();if(!src)continue;try{const r:any=await api.post("/translations/translate",{text:src,target_locale:activeLocale,source_locale:"en"});if(r?.translated_text){setTranslations(p=>({...p,[`${activeLocale}:${f.key}`]:r.translated_text}));await upsertTr(f.key,activeLocale,src,r.translated_text);count++;}}catch{}}
