...
export default function InventoryItemEditPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      for(const lc of LOCALES){if(lc.code==="en")continue;try{const rt=await api.getRaw<any>(`/admin/translations?table_name=inventory_items&record_id=${itemId}&locale=${lc.code}&per_page=50`);if(rt?.items)for(const t of rt.items){const f=t.translation_key.split(".").pop()||"";x[`${lc.code}:${f}`]=t.translated_text||"";}}catch (e) { console.error(e); }}
      setTranslations(x);
      ...
      {activeLocale==="en"?(
        <div className="card" style={{padding:24,maxWidth:700}}>
          <div className="df-grid">
            ...
            <div className="df-field"><label className="df-label">{`Unit Cost (${symbol})`}</label><input type="number" step="0.01" className="w-full border rounded px-3 py-2 text-sm" value={form.unit_cost} onChange={e=>setForm({...form,unit_cost:e.target.value})}/></div>
