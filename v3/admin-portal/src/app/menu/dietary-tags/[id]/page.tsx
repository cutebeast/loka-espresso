...
export default function DietaryTagEditPage() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await api.patch(`/admin/dietary-tags/${id}`, form);
      setMsg("Saved");
  ...
  const upsert = async (field: string, locale: string, src: string, text: string) => {
    ...
    if (ex) await api.put(`/admin/translations/${ex.id}`, {
      ...
      translation_key: `dietary_tags.${id}.${field}`,
      locale,
      namespace: "dietary",
      translated_text: text,
      source_text: src,
      table_name: "dietary_tags",
      record_id: Number(id),
      column_name: field
    });
  };
  ...
  if (loading) return <div style={{
    padding: 32
  }}>{t("menu_dietary-tags_[id].loading")}</div>;
  return <div style={{
    padding: 32
  }}>
      <div style={{
      ...
      gap: 12,
      marginBottom: 20
    }}><button onClick={() => r.push("/menu/dietary-tags")} className="btn btn-ghost btn-sm"><ArrowLeft size={18} /></button><div><h1 className="page-title" style={{
          margin: 0
        }}>{form.display_name || "Tag"}</h1></div></div>
      ...
      maxWidth: 600
          ...
          <div className="df-grid">
            <div className="df-field"><label className="form-label">{t("menu_dietary-tags_[id].display_name")}</label><input value={form.display_name || ""} onChange={e => setForm({
            ...form,
            display_name: e.target.value
          })} /></div>
            <div className="df-field"><label className="form-label">{t("menu_dietary-tags_[id].key")}</label><input value={form.tag_key || ""} onChange={e => setForm({
            ...form,
          ...
          gridColumn: "1/-1"
              ...
              <label className="form-label">{t("menu_dietary-tags_[id].icon")}</label>
              <div style={{
            display: "flex",
            alignItems: "center",
            gap: 8
              ...
              cursor: "pointer"
                ...
                <input style={{
              flex: 1
            }} value={form.icon || ""} onChange={e => setForm({
              ...form,
              icon: e.target.value
            })} placeholder={t("menu_dietary-tags_[id].or_type_emoji")} />
              </div>
              {showEmoji && <div style={{
            marginTop: 8,
            display: "flex",
            ...
            borderRadius: "var(--radius-sm)"
          }}>{EMOJIS.map(e => <button key={e} type="button" onClick={() => {
              setForm({
                ...form,
                icon: e
              });
              ...
              borderRadius: "var(--radius-sm)",
              cursor: "pointer",
              background: form.icon === e ? "rgba(59,74,26,0.15)" : "white"
            }}>{e}</button>)}</div>}
            </div>
            <div className="df-field"><label className="form-label">{t("menu_dietary-tags_[id].color")}</label><div style={{
            display: "flex",
            alignItems: "center",
            gap: 8
          }}><input type="color" value={form.color_hex || "#22C55E"} onChange={e => setForm({
              ...form,
            ...
            })} />{t("menu_dietary-tags_[id].active")}</label></div>
          </div>
          <div className="df-actions" style={{
        marginTop: 16
      }}><button type="button" onClick={() => r.push("/menu/dietary-tags")} className="btn btn-ghost">{t("menu_dietary-tags_[id].cancel")}</button><button onClick={save} disabled={saving} className="btn btn-primary"><Save size={16} /> {saving ? "Saving..." : "Save"}</button></div>
      ...
      maxWidth: 600
        ...
        marginBottom: 20
          ...
          margin: 0
        }}>{L.find(l => l.code === loc)?.flag} {L.find(l => l.code === loc)?.label}{t("menu_dietary-tags_[id].translation")}</h3><button onClick={regenAll} disabled={regen} className="btn btn-primary btn-sm" aria-label={t("menu_dietary-tags_[id].refresh")}><RefreshCw size={14} /> {regen ? "..." : "Regenerate All"}</button></div>
          ...
          return <div className="df-field" key={f.key}><label style={{
              ...
              color: "var(--color-text-muted)"
                ...
                fontStyle: "italic",
                marginLeft: 8
              }}>{t("menu_dietary-tags_[id].en")}{(form[f.key] || "").slice(0, 30)}</span></label><input value={tr[k] || ""} onChange={e => setTr(p => ({
              ...p,
              [k]: e.target.value
            }))} style={{
              width: "100%",
          ...
          <div style={{
        marginTop: 20
      }}><button onClick={saveAllTr} className="btn btn-primary"><Save size={16} />{t("menu_dietary-tags_[id].save_translations")}</button></div>
        </div>}
    </div>;
}