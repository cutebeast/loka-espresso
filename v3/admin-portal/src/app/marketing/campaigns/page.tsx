  ...
  return (
    <div style={{ padding: 32 }}>
      <div className="page-header">
        ...
        <button onClick={() => router.push("/marketing/campaigns/new")} className="btn btn-primary btn-sm"><Plus size={16} /> Add Campaign</button>
      </div>
        ...
        <tbody>
          ...
          : items.map(item => (
            <tr key={item.id} className="clickable" role="button" tabIndex={0} onKeyDown={(e)=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();(() => router.push(`/marketing/campaigns/${item.id}`))();}}} onClick={() => router.push(`/marketing/campaigns/${item.id}`)} style={{ cursor: "pointer" }}>
              ...
              <td style={{ fontWeight: 600 }}>RM {item.budget_spent.toFixed(2)}</td>
              <td onClick={e => e.stopPropagation()}>
                <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                  <button onClick={() => router.push(`/marketing/campaigns/${item.id}`)} className="btn btn-ghost btn-sm" style={{ color: "var(--color-info)" }}><Edit2 size={14} /></button>
                  {(item.status === "draft" || item.status === "scheduled") && <button onClick={() => handleSend(item.id)} className="btn btn-sm btn-primary" style={{ fontSize: 11 }}>Send</button>}
