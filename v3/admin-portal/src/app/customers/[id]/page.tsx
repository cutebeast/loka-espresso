  ...
  return (
    <div style={{ padding: 32 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        <button onClick={() => r.push("/customers")} className="btn btn-ghost btn-sm"><ArrowLeft size={18}/></button>
        <div style={{ flex: 1 }}>
      ...
      {tab === "profile" && (
        <div className="card" style={{ padding: 24, maxWidth: 600 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
            ...
            <button onClick={() => setEdit(!edit)} className="btn btn-sm btn-ghost" style={{ color: "var(--color-info)" }}>{edit ? "Cancel" : "Edit"}</button>
          </div>
          {edit ? (
            <div className="df-grid">
              <div className="df-field"><label className="df-label">Name</label><input value={form.display_name} onChange={e => setForm({...form, display_name: e.target.value})}/></div>
              ...
              <div className="df-field"><label className="df-label">Email</label><input value={form.email_address} onChange={e => setForm({...form, email_address: e.target.value})}/></div>
              <div className="df-field"><label className="df-label">DOB</label><input type="date" value={form.date_of_birth} onChange={e => setForm({...form, date_of_birth: e.target.value})}/></div>
              ...
              <div className="df-actions" style={{ gridColumn: "1/-1" }}><button onClick={handleSave} disabled={saving} className="btn btn-primary"><Save size={16}/>{saving?"Saving...":"Save"}</button></div>
            </div>
          ) : (
      ...
      {tab === "manage" && (
        <div style={{ maxWidth: 600, display: "flex", flexDirection: "column", gap: 16 }}>
          {!c.phone_verified_at && (
            <div className="card" style={{ padding: 16, borderLeft: "4px solid var(--color-warning, #F59E0B)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button onClick={() => doAction("approve", {})} disabled={!!actionLoading} className="btn btn-sm btn-primary">{actionLoading==="approve"?"...":"Approve"}</button>
              </div>
            </div>
          )}

          <div className="card" style={{ padding: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><h4 style={{ margin: 0 }}>Award / Deduct Points</h4><span style={{ fontSize: 22, fontWeight: 700, color: "var(--color-primary)" }}>{(c.loyalty?.points_balance||0).toLocaleString()} pts</span></div>
            <div style={{ display: "flex", gap: 8, alignItems: "end", flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 100 }}><label style={{ fontSize: 11, color: "var(--color-text-muted)", display: "block", marginBottom: 4 }}>Amount (+/-)</label><input type="number" value={pointsAmt} onChange={e => setPointsAmt(e.target.value)} placeholder="e.g. 100 or -50" style={{ width: "100%" }}/></div>
              <div style={{ flex: 2, minWidth: 150 }}><label style={{ fontSize: 11, color: "var(--color-text-muted)", display: "block", marginBottom: 4 }}>Reason</label><input value={pointsReason} onChange={e => setPointsReason(e.target.value)} placeholder="e.g. Loyalty bonus" style={{ width: "100%" }}/></div>
              <button onClick={() => doAction("points", { points: Number(pointsAmt), reason: pointsReason })} disabled={!!actionLoading} className="btn btn-sm btn-primary">{actionLoading==="points"?"...":"Apply"}</button>
            </div>
          </div>
          ...
          <div className="card" style={{ padding: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "end", flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 100 }}><label style={{ fontSize: 11, color: "var(--color-text-muted)", display: "block", marginBottom: 4 }}>Amount (+/-)</label><input type="number" step="0.01" value={walletAmt} onChange={e => setWalletAmt(e.target.value)} placeholder="+/- RM" style={{ width: "100%" }}/></div>
              <div style={{ flex: 2, minWidth: 150 }}><label style={{ fontSize: 11, color: "var(--color-text-muted)", display: "block", marginBottom: 4 }}>Reason</label><input value={walletReason} onChange={e => setWalletReason(e.target.value)} placeholder="e.g. Refund" style={{ width: "100%" }}/></div>
              <button onClick={() => doAction("wallet", { amount: walletAmt, reason: walletReason })} disabled={!!actionLoading} className="btn btn-sm btn-primary">{actionLoading==="wallet"?"...":"Apply"}</button>
            </div>
          </div>
          ...
          <div className="card" style={{ padding: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "end", flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 150 }}><label style={{ fontSize: 11, color: "var(--color-text-muted)", display: "block", marginBottom: 4 }}>Voucher</label><select value={selVoucher} onChange={e => setSelVoucher(e.target.value)} style={{ width: "100%" }}><option value="">Select...</option>{vouchers.map((v: any) => <option key={v.id} value={v.id}>{v.display_title || v.voucher_code}</option>)}</select></div>
              <div style={{ flex: 2, minWidth: 150 }}><label style={{ fontSize: 11, color: "var(--color-text-muted)", display: "block", marginBottom: 4 }}>Reason</label><input value={voucherReason} onChange={e => setVoucherReason(e.target.value)} placeholder="e.g. Compensation" style={{ width: "100%" }}/></div>
              <button onClick={() => doAction("voucher", { voucher_id: selVoucher, reason: voucherReason })} disabled={!!actionLoading || !selVoucher} className="btn btn-sm btn-primary">{actionLoading==="voucher"?"...":"Award"}</button>
            </div>
          </div>
          ...
          <div className="card" style={{ padding: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "end", flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 120 }}><label style={{ fontSize: 11, color: "var(--color-text-muted)", display: "block", marginBottom: 4 }}>Tier</label><select value={tierVal} onChange={e => setTierVal(e.target.value)} style={{ width: "100%" }}>{TIERS.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
              <div style={{ flex: 2, minWidth: 150 }}><label style={{ fontSize: 11, color: "var(--color-text-muted)", display: "block", marginBottom: 4 }}>Reason</label><input value={tierReason} onChange={e => setTierReason(e.target.value)} placeholder="e.g. VIP upgrade" style={{ width: "100%" }}/></div>
              <button onClick={() => doAction("tier", { tier: tierVal, reason: tierReason })} disabled={!!actionLoading} className="btn btn-sm btn-primary">{actionLoading==="tier"?"...":"Set"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === "orders" && (
        <PaginatedTable data={orders} cols={["Order #","Status","Total","Date"]} render={(o: any) => {
          ...
          return <><td className="font-mono" style={{ fontSize: 11 }}>{o.order_number}</td><td><span className={`badge badge-sm ${sm[o.status] || "badge-gray"}`}>{o.status?.replace(/_/g," ")}</span></td><td style={{ textAlign: "right" }}>{fmt(o.total_amount)}</td><td style={{ fontSize: 12 }}>{dt(o.created_at)}</td></>;
      ...
      {tab === "vouchers" && (<>
        ...
        <div className="table-container" style={{ marginBottom: 20 }}><table className="data-table"><thead><tr><th>Code</th><th>Status</th><th>Points</th><th>Expires</th></tr></thead><tbody>
          ...
          : rewardsVouchers.rewards.map((r: any) => (<tr key={r.id}><td className="font-mono" style={{ fontSize: 11 }}>{r.redemption_code}</td><td><span className="badge badge-sm badge-blue">{r.status}</span></td><td>{r.points_spent} pts</td><td style={{ fontSize: 12 }}>{dt(r.expires_at)}</td></tr>))}
        </tbody></table></div>
        ...
        <div className="table-container"><table className="data-table"><thead><tr><th>Code</th><th>Title</th><th>Status</th><th>Expires</th></tr></thead><tbody>
          ...
          : rewardsVouchers.vouchers.map((v: any) => (<tr key={v.id}><td className="font-mono" style={{ fontSize: 11 }}>{v.redemption_code}</td><td>{v.voucher_title || v.voucher_code}</td><td><span className="badge badge-sm badge-blue">{v.status}</span></td><td style={{ fontSize: 12 }}>{dt(v.expires_at)}</td></tr>))}
        </tbody></table></div>
      </>)}
      ...
      {tab === "consents" && (
        <div className="table-container"><table className="data-table"><thead><tr><th>Type</th><th style={{ width: 90 }}>Status</th><th>Granted</th></tr></thead><tbody>
          ...
          : c.consents.map((x: any) => (<tr key={x.id}><td style={{ textTransform: "capitalize" }}>{x.consent_type?.replace(/_/g," ")}</td><td><span className={`badge badge-sm ${x.status==="granted"?"badge-green":"badge-red"}`}>{x.status}</span></td><td style={{ fontSize: 12 }}>{x.granted_at ? new Date(x.granted_at).toLocaleString() : "—"}</td></tr>))}
        </tbody></table></div>
      )}
  ...
  return <div>
    ...
    {(data.total_pages||0) > 1 && (
      <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 12 }}>
        <button className="btn btn-sm btn-ghost" disabled={data.page <= 1} onClick={() => onChange(data.page - 1)}>← Prev</button>
        <span style={{ fontSize: 12, color: "var(--color-text-muted)" }}>{data.page}/{data.total_pages}</span>
        <button className="btn btn-sm btn-ghost" disabled={data.page >= data.total_pages} onClick={() => onChange(data.page + 1)}>Next →</button>
      </div>
    )}
  </div>;
}
