...
export default function VouchersPage() {
  ...
  const fetchData = useCallback(async (p: number = 1) => {
    setLoading(true);
    try {
      const d = await api.getRaw<{ items: Voucher[]; total: number; total_pages: number }>(
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await api.del(`/admin/vouchers/${id}`); fetchData(page); } catch { /* ignore */ }
  };
      ...
      <div className="table-container"><table className="data-table">
        ...
        <tbody>
          {loading ? <tr><td colSpan={8} className="data-table-empty">Loading...</td></tr>
            : items.map(item => (
              <tr key={item.id} className="clickable" onClick={() => router.push(`/vouchers/${item.id}`)} style={{ cursor: "pointer" }}>
