...
export default function InfoCardEditPage() {
  ...
  const load = useCallback(async () => {
    try{
      ...
      setForm({title:d.title||"",short_description:d.short_description||"",long_description:d.long_description||"",image_url:d.image_url||"",image_gallery_urls:d.image_gallery_urls||[],gallery_video_url:d.gallery_video_url||"",action_type:d.action_type||"",action_url:d.action_url||"",position:d.position||0,start_date:d.start_date?.slice(0,10)||"",end_date:d.end_date?.slice(0,10)||"",is_active:d.is_active,slug:d.slug||""});
      setImg(d.image_url||"");
      const x:Record<string,string>={};
      for(const lc of LOCALES){if(lc.code==="en")continue;try{const rt=await api.getRaw<any>(`/admin/translations?table_name=information_cards&record_id=${id}&locale=${lc.code}&per_page=50`);if(rt?.items)for(const t of rt.items){const f=t.translation_key.split(".").pop()||"";x[`${lc.code}:${f}`]=t.translated_text||"";}}catch (e) { console.error(e); }}
      setTr(x);
...
load();
  ...
  const save = async () => { setSaving(true);
    try{await api.patch(`/admin/info-cards/${id}`,form);setMsg("Saved");setTimeout(()=>setMsg(""),2000);}catch (e) { console.error(e); }finally{setSaving(false)}; };

  const upsertTr = async(field:string,locale:string,src:string,text:string)=>{
    try{const rt=await api.getRaw<any>(`/admin/translations?table_name=information_cards&record_id=${id}&column_name=${field}&locale=${locale}&per_page=1`);const ex=rt?.items?.[0];if(ex)await api.put(`/admin/translations/${ex.id}`,{translated_text:text});else await api.post("/admin/translations",{translation_key:`information_cards.${id}.${field}`,locale,namespace:"content",translated_text:text,source_text:src,table_name:"information_cards",record_id:Number(id),column_name:field});}catch (e) { console.error(e); }
  };
  ...
  return (
    <div style={{padding:32}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}><button onClick={()=>r.push("/content/info-cards")} className="btn btn-ghost btn-sm"><ArrowLeft size={18}/></button><div><h1 className="page-title" style={{margin:0}}>{form.title||"Info Card"}</h1></div></div>
      ...
      {loc==="en"?(
        <div className="card" style={{padding:24,maxWidth:700}}>
          ...
          <div className="df-grid">
            ...
            <div className="df-field"><label className="df-label">Action URL</label><input value={form.action_url} onChange={e=>setForm({...form,action_url:e.target.value})}/></div>
          ...
          <div className="df-actions" style={{marginTop:20}}><button type="button" onClick={()=>r.push("/content/info-cards")} className="btn btn-ghost">Cancel</button><button onClick={save} disabled={saving} className="btn btn-primary"><Save size={16}/>{saving?"Saving...":"Save Changes"}</button></div>
        </div>
      ):(
        <div className="card" style={{padding:24,maxWidth:700}}>
