...
export default function SplashScreensPage() {
  ...
  const fetch = () => { setLoading(true); api.get<{items:Screen[]}>("/admin/content/splash-screens?per_page=100").then(d => setItems(Array.isArray(d)?d:(d.items||[]))).catch(e=>setError(e.message)).finally(()=>setLoading(false)); };
  useEffect(()=>{fetch();},[]);
  ...
  return (
    <div style={{padding:32}}>
      ...
      <div className="table-container"><table className="data-table"><thead><tr><th>Title</th><th>Key</th><th>Period</th><th style={{width:80}}>Status</th><th style={{width:80}}>Actions</th></tr></thead><tbody>
        ...
        :items.map(item=>(<tr key={item.id} className="clickable" onClick={()=>router.push(`/content/pwa-splash/${item.id}`)} style={{cursor:"pointer"}}>
