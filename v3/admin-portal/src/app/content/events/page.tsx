...
export default function EventsPage() {
  ...
  const fetch = useCallback(() => {
    api.get<{items:Card[]}>("/admin/event-cards?per_page=100")
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await api.del(`/admin/event-cards/${id}`); fetch(); }
    ...
    <div style={{ padding: 32 }}>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{items.length} events</p>
        </div>
          ...
          <tbody>
            {loading ? (
              ...
              <tr
                ...
                role="button"
                tabIndex={0}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); router.push(`/content/events/${item.id}`); } }}
                onClick={() => router.push(`/content/events/${item.id}`)}
