...
export default function EventNewPage() {
  ...
  const [form, setForm] = useState({ title:"", short_description:"", long_description:"", image_url:"", position:0, is_active:true, start_date:"", end_date:"", location:"", event_datetime:"", rsvp_enabled:false, rsvp_max_capacity:0 });
  ...
  const handleSubmit = async (e:React.FormEvent) => { e.preventDefault(); setSaving(true);
    const pl:any={...form,position:Number(form.position),rsvp_max_capacity:form.rsvp_max_capacity||null};
    try{await api.post("/admin/event-cards",pl);router.push("/content/events");}catch(e:any){setError(e.message)}finally{setSaving(false)}; };

  return (
    <div style={{padding:32}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}><button onClick={()=>router.push("/content/events")} className="btn btn-ghost btn-sm"><ArrowLeft size={18}/></button><div><h1 className="page-title" style={{margin:0}}>New Event</h1></div></div>
      ...
      <div className="card" style={{padding:24,maxWidth:700}}>
        <form onSubmit={handleSubmit}><div className="df-grid">
          ...
          <div className="df-field"><label className="df-label">Location</label><input value={form.location} onChange={e=>setForm({...form,location:e.target.value})} placeholder="Loka Espresso KLCC"/></div>
          ...
          {form.rsvp_enabled&&<div className="df-field"><label className="df-label">Max Capacity</label><input type="number" value={form.rsvp_max_capacity} onChange={e=>setForm({...form,rsvp_max_capacity:Number(e.target.value)})} placeholder="e.g. 20"/></div>}
        ...
        </div><div className="df-actions" style={{marginTop:20}}><button type="button" onClick={()=>router.push("/content/events")} className="btn btn-ghost">Cancel</button><button type="submit" className="btn btn-primary" disabled={saving}><Save size={16}/>{saving?"Creating...":"Create Event"}</button></div></form>
      </div>
    </div>
  );
}
