...
export default function ProductEditPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      for (const lc of LOCALES) {
        ...
        try {
          const rt = await api.getRaw<any>(`/admin/translations?table_name=product_cards&record_id=${id}&locale=${lc.code}&per_page=50`);
  ...
  const save = async () => {
    ...
    try {
      ...
      await api.patch(`/admin/product-cards/${id}`, pl);
      setMsg("Saved");
  ...
  const upsertTr = async (field: string, locale: string, src: string, text: string) => {
    try {
      const rt = await api.getRaw<any>(`/admin/translations?table_name=product_cards&record_id=${id}&column_name=${field}&locale=${locale}&per_page=1`);
      const ex = rt?.items?.[0];
      if (ex) await api.put(`/admin/translations/${ex.id}`, {
        translated_text: text
      });else await api.post("/admin/translations", {
        translation_key: `product_cards.${id}.${field}`,
        locale,
        namespace: "content",
        translated_text: text,
        source_text: src,
        table_name: "product_cards",
        record_id: Number(id),
        column_name: field
      });
    } catch (e) {
  ...
  if (loading) return <div style={{
    padding: 32
  }}>{t("content_products_[id].loading")}</div>;
  return <div style={{
    padding: 32
  }}>
      <div style={{
      ...
      gap: 12,
      marginBottom: 20
    }}><button onClick={() => r.push("/content/products")} className="btn btn-ghost btn-sm"><ArrowLeft size={18} /></button><div><h1 className="page-title" style={{
          margin: 0
        }}>{form.title || "Product"}</h1></div></div>
      ...
      maxWidth: 700
          ...
          <h3 style={{
        marginBottom: 20
      }}>{t("content_products_[id].english_source_content")}</h3>
          <div className="df-grid">
            <div className="df-field" style={{
          gridColumn: "1/-1"
        }}><label className="df-label">{t("content_products_[id].title")}</label><input required value={form.title} onChange={e => setForm({
            ...form,
            ...
            <div className="df-field"><label className="df-label">{t("content_products_[id].position")}</label><input type="number" value={form.position} onChange={e => setForm({
            ...form,
            position: Number(e.target.value)
          })} /></div>
            <div className="df-field" style={{
          gridColumn: "1/-1"
        }}><label className="df-label">{t("content_products_[id].short_description")}</label><input value={form.short_description} onChange={e => setForm({
            ...form,
            short_description: e.target.value
          })} /></div>
            <div className="df-field" style={{
          gridColumn: "1/-1"
        }}><label className="df-label">{t("content_products_[id].full_description")}</label><textarea rows={3} value={form.long_description} onChange={e => setForm({
            ...form,
            long_description: e.target.value
          })} /></div>
            <div className="df-field" style={{
          gridColumn: "1/-1"
        }}><label className="df-label">{t("content_products_[id].image")}</label><div style={{
            display: "flex",
            gap: 12,
            alignItems: "center"
              ...
              }}>{t("content_products_[id].clear")}</button></>}</div></div>
          ...
          gallery_video_url: url
            ...
            })} />{t("content_products_[id].active")}</label></div>
          </div>
          <div className="df-actions" style={{
        marginTop: 20
      }}><button type="button" onClick={() => r.push("/content/products")} className="btn btn-ghost">{t("content_products_[id].cancel")}</button><button onClick={save} disabled={saving} className="btn btn-primary"><Save size={16} />{saving ? "Saving..." : "Save Changes"}</button></div>
      ...
      maxWidth: 700
        ...
        marginBottom: 20
          ...
          margin: 0
        }}>{LOCALES.find(l => l.code === loc)?.flag} {LOCALES.find(l => l.code === loc)?.label}{t("content_products_[id].translation")}</h3><button onClick={regenAll} disabled={regen} className="btn btn-primary btn-sm" aria-label={t("content_products_[id].refresh")}><RefreshCw size={14} />{regen ? "..." : "Regenerate All"}</button></div>
          ...
          return <div className="df-field" key={f.key} style={isLong ? {
            gridColumn: "1/-1"
              ...
              color: "var(--color-text-muted)"
                ...
                fontStyle: "italic",
                marginLeft: 8
              }}>{t("content_products_[id].en")}{(form[f.key] || "").slice(0, 40)}</span></label>{isLong ? <textarea rows={3} value={tr[k] || ""} onChange={e => setTr(p => ({
              ...p,
              [k]: e.target.value
            }))} style={{
              width: "100%",
          ...
          <div style={{
        marginTop: 20
      }}><button onClick={saveAllTr} disabled={savingTr} className="btn btn-primary"><Save size={16} />{t("content_products_[id].save_translations")}</button></div>
        </div>}
    </div>;
}