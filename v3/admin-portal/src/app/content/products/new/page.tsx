...
export default function ProductNewPage() {
  ...
  const [form, setForm] = useState({ title:"", short_description:"", long_description:"", image_url:"", price:0, position:0, is_active:true, start_date:"", end_date:"" });
  ...
  const handleSubmit = async (e:React.FormEvent) => { e.preventDefault(); setSaving(true);
    try{await api.post("/admin/product-cards",form);router.push("/content/products");}catch(e:any){setError(e.message)}finally{setSaving(false)}; };

  return (
    <div style={{padding:32}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}><button onClick={()=>router.push("/content/products")} className="btn btn-ghost btn-sm"><ArrowLeft size={18}/></button><div><h1 className="page-title" style={{margin:0}}>New Product</h1></div></div>
      ...
      <div className="card" style={{padding:24,maxWidth:700}}>
        ...
        </div><div className="df-actions" style={{marginTop:20}}><button type="button" onClick={()=>router.push("/content/products")} className="btn btn-ghost">Cancel</button><button type="submit" className="btn btn-primary" disabled={saving}><Save size={16}/>{saving?"Creating...":"Create Product"}</button></div></form>
      </div>
    </div>
  );
}
