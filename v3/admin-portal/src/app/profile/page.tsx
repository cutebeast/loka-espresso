...
export default function AdminProfilePage() {
  ...
  const handlePasswordChange = async () => {
    ...
    if (!newPw || newPw.length < 8) { setMsg("Password must be at least 8 characters"); setIsError(true); return; }
    setSaving(true);
    try {
      await api.post("/admin/auth/change-password", { current_password: currentPw, password: newPw });
  ...
  return (
    <div style={{ padding: 32 }}>
      <h1 className="page-title">My Profile</h1>
      <p className="page-subtitle" style={{ marginBottom: 24 }}>View your account details and update your password</p>
      ...
      <div className="card" style={{ maxWidth: 560, marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
          <div style={{ width: 56, height: 56, borderRadius: "50%", background: "var(--color-primary)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: 22, fontWeight: 700, flexShrink: 0 }}>
            {profile.display_name?.charAt(0)?.toUpperCase() || "A"}
          </div>
          <div>
            <h2 style={{ margin: 0, fontSize: 20 }}>{profile.display_name}</h2>
            <p style={{ margin: "4px 0 0", color: "var(--color-text-muted)", fontSize: 14 }}>{profile.email}</p>
          </div>
        </div>
        ...
        <table className="data-table">
          <tbody>
            <tr>
              <td style={{ fontWeight: 600, width: 140 }}><Mail size={14} style={{ marginRight: 6 }} />Email</td>
              <td>{profile.email}</td>
            </tr>
            <tr>
              <td style={{ fontWeight: 600 }}><Shield size={14} style={{ marginRight: 6 }} />Roles</td>
            ...
            <tr>
              <td style={{ fontWeight: 600 }}>MFA</td>
              <td>
            ...
            {profile.store_ids.length > 0 && (
              <tr>
                <td style={{ fontWeight: 600 }}>Store Access</td>
        ...
        {showPw ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button type="button" className="btn btn-outline" onClick={() => { setShowPw(false); setCurrentPw(""); setNewPw(""); }}>Cancel</button>
            </div>
          </div>
        ) : (
          <button type="button" className="btn btn-outline" onClick={() => setShowPw(true)}>
