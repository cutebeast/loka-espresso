...
export default function TranslationPage() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      }>("/admin/translations/sync-to-json");
  ...
  return <div style={{
    padding: 24
      ...
      alignItems: "flex-start"
        ...
        <button type="button" onClick={handleSync} disabled={syncing} className="btn btn-sm btn-outline" style={{
        display: "flex",
        alignItems: "center",
        gap: 6
      }} title={t("translations.sync_db_translations_to_pwa_json")}>
          <RefreshCw size={14} className={syncing ? "animate-spin" : ""} />
          {syncing ? t("admin.translations.syncing") : t("admin.translations.syncToJson")}
        </button>
      </div>

      {syncMsg && <div className="alert alert-success" style={{
      marginBottom: 12,
      fontSize: 13
    }}>{syncMsg}</div>}
