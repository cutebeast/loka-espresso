...
const ALL_FIELDS: {
  ...
  fields: [{
    ...
    key: "first_order_minutes_after_open",
    label: "First Order After Open (min)",
    type: "number"
  }, {
    key: "last_order_minutes_before_close",
    label: "Last Order Before Close (min)",
    type: "number"
  }]
}, {
  section: "Media",
...
export default function StoreEditPage() {
  ...
  const loadStore = async () => {
    ...
    try {
      ...
      const results = await Promise.all(LOCALES.filter(loc => loc.code !== "en").map(async loc => {
        try {
  ...
  if (loading) return <div style={{
    padding: 32
  }}><p>{t("stores_[id].loading")}</p></div>;
  if (!store) return <div style={{
    padding: 32
  }}><p>{t("stores_[id].store_not_found")}</p></div>;
  return <div style={{
    padding: 32
  }}>
      {/* Header */}
      ...
      marginBottom: 20
        ...
        <div>
          ...
          marginTop: 2
        }}>{t("stores_[id].edit_store_details_translations")}</p>
        </div>
      </div>
      ...
      maxWidth: 720
          ...
          <h3 style={{
        marginBottom: 20
      }}>{t("stores_[id].english_source_content")}</h3>
          {ALL_FIELDS.map(section => <div key={section.section} className={sectionClass}>
              <div className={sectionTitle}>{section.section}</div>
              <div className="df-grid">
                {section.fields.map(field => {
            if (field.type === "checkbox") {
              return <div className="df-field" key={field.key}>
                        <label style={{
                  display: "flex",
                  alignItems: "center",
                  gap: 8,
                  fontSize: 13
                          ...
                          <input type="checkbox" checked={!!form[field.key]} onChange={e => setForm({
                    ...form,
            ...
            return <div className="df-field" key={field.key}>
                      ...
                      <input type={field.type || "text"} value={form[field.key] != null ? form[field.key] : ""} onChange={e => setForm({
                ...form,
                [field.key]: e.target.value
              })} maxLength={field.type === "country" ? 2 : undefined} placeholder={field.type === "country" ? "e.g. MY" : ""} />
                    </div>;
          })}
              </div>
            </div>)}
          ...
          <div className={sectionClass}>
            <div className={sectionTitle}>{t("stores_[id].operating_hours")}</div>
          ...
          flexWrap: "wrap"
            ...
            marginLeft: 4
              ...
              setHours(hh);
            }} />{t("stores_[id].24h")}</label>
                {!h.is_closed && !h.is_24_hours && <label style={{
            display: "flex",
            alignItems: "center",
            gap: 4,
            fontSize: 12,
            ...
            marginLeft: 4
                    ...
                    <span style={{
              color: "var(--color-text-muted)"
            }}>{t("stores_[id].last_order_2")}</span>
                    <input type="time" value={h.last_order_time || ""} onChange={e => {
              const hh = [...hours];
              hh[i] = {
                ...hh[i],
          ...
          {/* Special Hours */}
          <div className={sectionClass}>
            <div className={sectionTitle}>{t("stores_[id].special_hours_holidays_events")}</div>
            {specialHours.map((sh: any, i: number) => <div key={i} style={{
          display: "flex",
          gap: 8,
          alignItems: "center",
          marginBottom: 6,
          flexWrap: "wrap",
          opacity: sh.is_closed ? 0.5 : 1
        }}>
                <input type="date" value={sh.special_date || ""} onChange={e => {
            const hh = [...specialHours];
            hh[i] = {
              ...hh[i],
            ...
            width: 140
          }} title={t("stores_[id].date")} />
                <input type="text" value={sh.reason || ""} onChange={e => {
            const hh = [...specialHours];
            hh[i] = {
              ...hh[i],
            ...
            width: 100
          }} placeholder={t("stores_[id].reason")} />
                <input type="time" value={sh.open_time || ""} onChange={e => {
            const hh = [...specialHours];
            hh[i] = {
              ...hh[i],
              open_time: e.target.value
            ...
            width: 110
          }} disabled={sh.is_closed} title={t("stores_[id].open")} />
				<span style={{
            fontSize: 12
          }}>{t("stores_[id].to_2")}</span>
                <input type="time" value={sh.close_time || ""} onChange={e => {
            const hh = [...specialHours];
            hh[i] = {
              ...hh[i],
              close_time: e.target.value
            ...
            width: 110
          }} disabled={sh.is_closed} title={t("stores_[id].close")} />
                <label style={{
            display: "flex",
            alignItems: "center",
            gap: 4,
            fontSize: 12
          }}><input type="checkbox" checked={sh.is_closed} onChange={e => {
              const hh = [...specialHours];
              hh[i] = {
                ...hh[i],
                is_closed: e.target.checked
              ...
              setSpecialHours(hh);
            }} />{t("stores_[id].closed_2")}</label>
                <button type="button" onClick={() => setSpecialHours(shs => shs.filter((_s, idx) => idx !== i))} className="btn btn-ghost btn-sm" style={{
            color: "var(--color-error)",
            fontSize: 11
          }}>{t("stores_[id].remove")}</button>
              </div>)}
            <button type="button" onClick={() => setSpecialHours([...specialHours, {
          special_date: "",
          reason: "",
          open_time: "",
          close_time: "",
          ...
          marginTop: 8
        }}>{t("stores_[id].add_special_date")}</button>
          </div>
          <div className="df-actions">
            <button type="button" onClick={() => router.push("/stores")} className="btn btn-ghost">{t("stores_[id].cancel")}</button>
      ...
      maxWidth: 720
        ...
        marginBottom: 20
          ...
          fontSize: 16
              ...
              {LOCALES.find(l => l.code === activeLocale)?.flag} {LOCALES.find(l => l.code === activeLocale)?.label}{t("stores_[id].translation")}</h3>
          ...
          marginBottom: 10
            ...
            return <div className="df-field" key={field.key}>
                ...
                justifyContent: "space-between"
                  ...
                  fontWeight: 400,
                  fontStyle: "italic"
                }}>{t("stores_[id].en")}{sourceText?.slice(0, 30) || "—"}</span>
                    </label>
                    <input value={translation} onChange={e => setTranslations(prev => ({
                ...prev,
        ...
        marginTop: 20
          ...
          padding: "10px 24px"
              ...
              <Save size={16} />{t("stores_[id].save_all_translations")}</button>
          </div>
        </div>}
    </div>;
}