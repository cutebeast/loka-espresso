...
const PERM_GROUPS: { label: string; resources: string[] }[] = [
  { label: "Company", resources: ["dashboard","stores","menu","loyalty","rewards","vouchers","surveys","promotions","campaigns"] },
  { label: "Content", resources: ["content"] },
  { label: "Settings", resources: ["settings"] },
  { label: "Admin User", resources: ["admin_users"] },
  { label: "Staff Portal", resources: ["staff_portal"] },
  { label: "Support", resources: ["customers","notifications","feedback"] },
  { label: "Store Operations", resources: ["orders","refunds","tables","inventory","staff","reservations"] },
  { label: "Reports & Other", resources: ["reports","translations","audit_log","referrals","checkins"] },
];
...
export default function RolesPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false); setForm({ display_name: "", description: "", role_key: "" });
      setMsg("Role created"); setTimeout(() => setMsg(""), 2000); fetch();
  ...
  const handleUpdate = async (id: number) => {
    setSaving(true);
    try {
      await api.put(`/admin/auth/roles/${id}`, { display_name: form.display_name, description: form.description });
      setEditingId(null); setMsg("Role updated"); setTimeout(() => setMsg(""), 2000); fetch();
    } catch (e: any) { setMsg(e.message || "Failed"); } finally { setSaving(false); }
  };
  ...
  const startEdit = (r: Role) => {
    setForm({ display_name: r.display_name, description: r.description || "", role_key: r.role_key });
    setEditingId(r.id);
  };

  return (
    <div style={{ padding: 24 }}>
      <div className="page-header"><div><h1 className="page-title">Roles & Permissions</h1><p className="page-subtitle">{roles.length} roles · {permissions.length} permissions</p></div><button onClick={() => { setShowForm(true); setForm({ display_name: "", description: "", role_key: "" }); }} className="btn btn-primary btn-sm"><Plus size={14} /> Create Role</button></div>
        ...
        <div className="card" style={{ padding: 20, marginBottom: 16, maxWidth: 440 }}>
          ...
          <form onSubmit={e => { e.preventDefault(); editingId ? handleUpdate(editingId) : handleCreate(e); }}>
            <div className="df-grid">
              ...
              <div className="df-field" style={{ gridColumn: "1/-1" }}><label className="form-label">Description</label><textarea className="w-full border rounded px-3 py-2 text-sm" rows={2} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="What this role can access" /></div>
            </div>
      ...
      {assigningRole && (
        <div className="card" style={{ padding: 20, marginBottom: 16, maxWidth: 700 }}>
          <h3 style={{ margin: "0 0 4px", display: "flex", alignItems: "center", gap: 6 }}><ShieldCheck size={18} /> Permissions — {roles.find(r => r.id === assigningRole)?.display_name}</h3>
          <p style={{ fontSize: 12, opacity: 0.5, margin: "0 0 16px" }}>Select which areas this role can access. Grouped by sidebar navigation.</p>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(190px, 1fr))", gap: 4 }}>
                    {groupPerms.map(p => (
                      <label key={p.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 8px", borderRadius: 6, border: "1px solid var(--color-border-light)", fontSize: 12, cursor: "pointer", background: rolePerms.includes(p.id) ? "rgba(59,74,26,0.04)" : "transparent" }}>
                        <input type="checkbox" checked={rolePerms.includes(p.id)} onChange={() => togglePerm(p.id)} style={{ margin: 0 }} />
          ...
          <div className="df-actions" style={{ marginTop: 16 }}><button type="button" onClick={() => setAssigningRole(null)} className="btn btn-ghost">Cancel</button><button onClick={savePerms} className="btn btn-primary"><Save size={14} /> Save Permissions</button></div>
        </div>
      )}
