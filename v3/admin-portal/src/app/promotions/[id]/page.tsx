...
export default function PromotionEditPage() {
  ...
  const loadRefs = async () => {
    try {
      const d = await api.getRaw<any>("/admin/vouchers?per_page=100&is_active=true");
      setVouchers(Array.isArray(d) ? d : d.items || []);
    } catch (e) {
      console.error(e);
    }
    try {
      const d = await api.getRaw<any>("/admin/surveys?per_page=100");
      setSurveys(Array.isArray(d) ? d : d.items || []);
    } catch (e) {
      console.error(e);
    }
  };
  const load = useCallback(async () => {
    try {
      ...
      setForm({
        ...
        voucher_id: String(d.voucher_id || ""),
        survey_id: String(d.survey_id || ""),
      ...
      for (const lc of LOCALES) {
        ...
        try {
          ...
          }>(`/admin/translations?table_name=promo_banners&record_id=${id}&locale=${lc.code}&per_page=50`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.patch(`/admin/promo-banners/${id}`, p);
      setMsg("Saved");
  ...
  const upsertTr = async (field: string, locale: string, src: string, text: string) => {
    ...
    }>(`/admin/translations?table_name=promo_banners&record_id=${id}&column_name=${field}&locale=${locale}&per_page=1`);
    if (all?.items?.[0]) {
      ...
      await api.post("/admin/translations", {
        translation_key: `promo_banners.${id}.${field}`,
        locale,
        namespace: "promo",
        translated_text: text,
        source_text: src,
        table_name: "promo_banners",
        record_id: Number(id),
        column_name: field
      });
    }
  ...
  if (loading) return <div style={{
    padding: 32
  }}><p>{t("promotions_[id].loading")}</p></div>;
  return <div style={{
    padding: 32
  }}>
      <div style={{
      ...
      marginBottom: 20
        ...
        <div><h1 className="page-title" style={{
          margin: 0
        }}>{form.title || "Promotion"}</h1><p className="page-subtitle" style={{
          marginTop: 2
        }}>{t("promotions_[id].edit_details_translations")}</p></div>
      </div>
      ...
      maxWidth: 700
        ...
        <h3 style={{
        marginBottom: 20
      }}>{t("promotions_[id].english_source_content")}</h3>
        <div className="df-grid">
          <div className="df-field" style={{
          gridColumn: "1/-1"
        }}><label className="df-label">{t("promotions_[id].title")}</label><input required value={form.title} onChange={e => setForm({
            ...form,
            title: e.target.value
          })} /></div>
          <div className="df-field"><label className="df-label">{t("promotions_[id].position")}</label><input type="number" value={form.position} onChange={e => setForm({
            ...form,
            position: Number(e.target.value)
          })} /></div>
          <div className="df-field" style={{
          gridColumn: "1/-1"
        }}><label className="df-label">{t("promotions_[id].short_description")}</label><input value={form.short_description} onChange={e => setForm({
            ...form,
            short_description: e.target.value
          })} /></div>
          <div className="df-field" style={{
          gridColumn: "1/-1"
        }}><label className="df-label">{t("promotions_[id].full_description")}</label><textarea rows={4} value={form.long_description} onChange={e => setForm({
            ...form,
            long_description: e.target.value
          })} /></div>
          <div className="df-field"><label className="df-label">{t("promotions_[id].action_type")}</label><select value={form.action_type} onChange={e => setForm({
            ...form,
            action_type: e.target.value
          })}><option value="">{t("promotions_[id].select")}</option>{ACTION_TYPES.map(a => <option key={a.value} value={a.value}>{a.label}</option>)}</select></div>
          <div className="df-field"><label className="df-label">{t("promotions_[id].reward_voucher")}</label><select required value={form.voucher_id} onChange={e => setForm({
            ...form,
            voucher_id: e.target.value
          })}><option value="">{t("promotions_[id].required")}</option>{vouchers.map(v => <option key={v.id} value={v.id}>{v.display_title} ({v.voucher_code})</option>)}</select>
          ...
          }}>{t("promotions_[id].this_voucher_is_restricted_to")}<strong>{voucherSegments.join(", ")}</strong></div>}</div>
          {form.action_type === "url_claim" && <div className="df-field" style={{
          gridColumn: "1/-1"
        }}><label className="df-label">{t("promotions_[id].link_url")}</label><input value={form.action_url} onChange={e => setForm({
            ...form,
            action_url: e.target.value
          })} /></div>}
          {form.action_type === "survey_claim" && <div className="df-field"><label className="df-label">{t("promotions_[id].survey")}</label><select value={form.survey_id} onChange={e => setForm({
            ...form,
            survey_id: e.target.value
          })}><option value="">{t("promotions_[id].select_2")}</option>{surveys.map(s => <option key={s.id} value={s.id}>{s.survey_name}</option>)}</select></div>}
          <div className="df-field" style={{
          gridColumn: "1/-1"
        }}>
            <label className="df-label">{t("promotions_[id].image")}</label>
            <div style={{
            display: "flex",
            gap: 12,
            alignItems: "center"
              ...
              }}>{t("promotions_[id].clear")}</button></>}</div>
          </div>
          ...
          <div className="df-field"><label className="df-label">{t("promotions_[id].start_date")}</label><input type="date" value={form.start_date} onChange={e => setForm({
            ...form,
            start_date: e.target.value
          })} /></div>
          <div className="df-field"><label className="df-label">{t("promotions_[id].end_date")}</label><input type="date" value={form.end_date} onChange={e => setForm({
            ...form,
          ...
          <div className="df-field"><label className="df-label" style={{
            ...
            })} />{t("promotions_[id].active")}</label></div>
        </div>
        <div className="df-actions" style={{
        marginTop: 20
      }}><button type="button" onClick={() => r.push("/promotions")} className="btn btn-ghost">{t("promotions_[id].cancel")}</button><button onClick={handleSave} disabled={saving} className="btn btn-primary"><Save size={16} /> {saving ? "Saving..." : "Save Changes"}</button></div>
      </div>}
      ...
      maxWidth: 720
        ...
        marginBottom: 20
          ...
          margin: 0,
          fontSize: 16
        }}>{LOCALES.find(l => l.code === loc)?.flag} {LOCALES.find(l => l.code === loc)?.label}{t("promotions_[id].translation")}</h3><button onClick={() => regenAll(loc)} disabled={!!regen} className="btn btn-primary btn-sm" style={{
          fontSize: 12,
          padding: "8px 16px"
          ...
          return <div className="df-field" key={f.key} style={isLong ? {
            gridColumn: "1/-1"
              ...
              justifyContent: "space-between"
                ...
                fontWeight: 400,
                fontStyle: "italic"
              }}>{_t("promotions_[id].en")}{s?.slice(0, 30) || "—"}</span></label>{isLong ? <textarea rows={3} value={t} onChange={e => setTr(prev => ({
              ...prev,
              [k]: e.target.value
            }))} style={{
              width: "100%",
        ...
        marginTop: 20
          ...
          fontSize: 13,
          padding: "10px 24px"
        }}><Save size={16} />{t("promotions_[id].save_all_translations")}</button></div>
      </div>}
    </div>;
}