...
export default function StaffPage() {
  ...
  const fetchStores = async () => {
    try { const d = await api.get<any>("/admin/stores?per_page=50"); const list = Array.isArray(d) ? d : (d.items || []); setStores(list); }
    catch {}
  };
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const d = await api.get<any>(`/admin/staff/roles`); // Use the roles endpoint that includes store_name + roles
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await api.del(`/admin/staff/${id}`); fetchData(); } catch {}
  };

  return (
    <div style={{ padding: 24 }}>
      <div className="page-header"><div><h1 className="page-title">Staff</h1><p className="page-subtitle">{items.length} members</p></div><button onClick={() => router.push("/staff/new")} className="btn btn-primary btn-sm"><Plus size={14} /> Add Staff</button></div>
        ...
        <tbody>
          ...
          : items.map(s => (
            <tr key={s.id}>
              ...
              <td style={{ fontSize: 12 }}>
                {s.roles?.length ? (
                  <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                    {s.roles.map((r: any) => <span key={r.id} className="badge badge-sm badge-primary" style={{ fontSize: 10 }}>{r.name}</span>)}
                  </div>
              ...
              <td>
                <button onClick={() => handleDelete(s.id)} className="btn btn-ghost btn-sm" style={{ color: "var(--color-error)" }}><Trash2 size={14} /></button>
              </td>
            </tr>
          ))}
        </tbody>
