...
export default function TemplateEditPage() {
  ...
  const load = useCallback(async () => {
    try{
      ...
      for(const lc of LOCALES){if(lc.code==="en")continue;try{const rt=await api.getRaw<any>(`/admin/translations?table_name=notification_templates&record_id=${id}&locale=${lc.code}&per_page=50`);if(rt?.items)for(const t of rt.items){const f=t.translation_key.split(".").pop()||"";x[`${lc.code}:${f}`]=t.translated_text||"";}}catch (e) { console.error(e); }}
      setTr(x);
...
load();
  ...
  const save = async () => { setSaving(true);
    try{await api.put(`/admin/notifications/templates/${id}`,form);setMsg("Saved");setTimeout(()=>setMsg(""),2000);}catch (e) { console.error(e); }finally{setSaving(false)}; };

  const upsertTr = async(field:string,locale:string,src:string,text:string)=>{
    try{const rt=await api.getRaw<any>(`/admin/translations?table_name=notification_templates&record_id=${id}&column_name=${field}&locale=${locale}&per_page=1`);const ex=rt?.items?.[0];if(ex)await api.put(`/admin/translations/${ex.id}`,{translated_text:text});else await api.post("/admin/translations",{translation_key:`notification_templates.${id}.${field}`,locale,namespace:"notification",translated_text:text,source_text:src,table_name:"notification_templates",record_id:Number(id),column_name:field});}catch (e) { console.error(e); }
  };
