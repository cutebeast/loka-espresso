...
const MAX_Q = 5;
interface SurveyQuestion { question_text: string; question_type: string; options: string[]; is_required: boolean; display_order: number; }
...
export default function SurveyNewPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const payload: any = { ...form, survey_key: form.survey_key || form.survey_name.toLowerCase().replace(/[^a-z0-9]/g,"_") };
      payload.questions = form.questions.map((q: any) => q.options && (q.question_type === "single_choice" || q.question_type === "multiple_choice") ? { ...q, answer_options: { choices: q.options } } : q);
  ...
  return (
    <div style={{ padding: 32 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <div><h1 className="page-title" style={{ margin: 0 }}>New Survey</h1><p className="page-subtitle" style={{ marginTop: 2 }}>Create a survey with up to 5 questions</p></div>
      </div>
      {error && <div className="alert alert-error" style={{ marginBottom: 16 }}>{error}</div>}

      <div className="card" style={{ padding: 24, maxWidth: 700 }}>
        <form onSubmit={handleSubmit}><div className="df-grid">
          <div className="df-field"><label className="df-label">Key</label><input value={form.survey_key} onChange={e => setForm({...form, survey_key: e.target.value})} placeholder="auto-generated" /></div>
        ...
        </div><div className="df-actions" style={{marginTop:20}}><button type="button" onClick={() => router.push("/surveys")} className="btn btn-ghost">Cancel</button><button type="submit" className="btn btn-primary" disabled={saving}><Save size={16} /> {saving?"Creating...":"Create Survey"}</button></div></form>
      </div>
    </div>
  );
}
