...
function prefixFromPath(relPath) {
  ...
  if (parts[0] === "app") {
    ...
    // area/page.tsx -> area; area/something/page.tsx -> area.something
...
function processFile(filePath, allKeys) {
          ...
          if (stmt.type === "VariableDeclaration") {
            for (const decl of stmt.declarations) {
              ...
              if (decl.init &&
                ...
                const hasT = decl.id.properties.some((p) => p.key && p.key.name === "t");
  ...
  const hasImport = ast.program.body.some(
    (stmt) => stmt.type === "ImportDeclaration" && stmt.source.value === "@/lib/i18n"
  );
  if (!hasImport) {
