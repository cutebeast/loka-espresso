...
down_revision = '82f8aa600119'
branch_labels = None
depends_on = None


def upgrade():
    if not au.table_exists('checkout_tokens'):
    ...
    if not au.column_exists('menu_items', 'dietary_tags'):
        op.add_column('menu_items', sa.Column('dietary_tags', postgresql.JSON(), nullable=True))

    if not au.column_exists('users', 'referral_count'):
        op.add_column('users', sa.Column('referral_count', sa.Integer(), nullable=False, server_default='0'))
    if not au.column_exists('users', 'referral_earnings'):
        op.add_column('users', sa.Column('referral_earnings', sa.DECIMAL(10, 2), nullable=False, server_default='0.00'))

    if not au.column_exists('referrals', 'referrer_reward_paid'):
        op.add_column('referrals', sa.Column('referrer_reward_paid', sa.Boolean(), nullable=False, server_default='false'))
    if not au.column_exists('referrals', 'referred_user_order_count'):
        op.add_column('referrals', sa.Column('referred_user_order_count', sa.Integer(), nullable=False, server_default='0'))


def downgrade():
    if au.column_exists('referrals', 'referred_user_order_count'):
        op.drop_column('referrals', 'referred_user_order_count')
    if au.column_exists('referrals', 'referrer_reward_paid'):
        op.drop_column('referrals', 'referrer_reward_paid')
    if au.column_exists('users', 'referral_earnings'):
        op.drop_column('users', 'referral_earnings')
    if au.column_exists('users', 'referral_count'):
        op.drop_column('users', 'referral_count')
    if au.column_exists('menu_items', 'dietary_tags'):
        op.drop_column('menu_items', 'dietary_tags')
    if au.index_exists('checkout_tokens', 'ix_checkout_tokens_user_id'):
        op.drop_index('ix_checkout_tokens_user_id', table_name='checkout_tokens')
    if au.index_exists('checkout_tokens', 'ix_checkout_tokens_token'):
        op.drop_index('ix_checkout_tokens_token', table_name='checkout_tokens')
    if au.table_exists('checkout_tokens'):
