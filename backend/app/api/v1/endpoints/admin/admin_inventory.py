...
async def _build_movement_out(m, store_id, db):
    user_result = await db.execute(select(AdminUser).where(AdminUser.id == m.created_by))
...
async def store_ledger(
    ...
    user_map = {}
    if user_ids:
        u_result = await db.execute(select(AdminUser).where(AdminUser.id.in_(user_ids)))
...
@router.post("/system/backfill-inventory-ledger", status_code=200)
