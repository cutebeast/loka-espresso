...
export default function EmergencyPopup() {
  ...
  const checkPopup = useCallback(async () => {
    try {
      const res = await api.get('/content/information?content_type=pop_up&limit=1');
  ...
  const imageUrl = resolveUrl(popup.image_url);
              ...
              {!imageUrl && (
                <div className="relative px-5 pt-5 pb-2">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-bold text-red-600 uppercase tracking-wider">Announcement</span>
                  </div>
                  <button
                    onClick={handleClose}
